...
The zero-count strip has no probability ordinate and supplies no upper bound.
"""
...
fig=plt.figure(figsize=(7.2047244094,6.1023622047))
grid=fig.add_gridspec(2,3,left=.102,right=.985,bottom=.14,top=.895,wspace=.32,hspace=.51,height_ratios=[1,1.04])
upper=[];lower=[];strips=[]
for col,run in enumerate(RUNS):
    ...
    gs=grid[1,col].subgridspec(2,1,height_ratios=[1,.16],hspace=.08)
    ax=fig.add_subplot(gs[0]);z=fig.add_subplot(gs[1],sharex=ax);lower.append(ax);strips.append(z)
    ...
    z.set_facecolor('#F3F3F3');z.set_ylim(0,1)
    z.scatter(ranks[k==0],np.full((k==0).sum(),.5),marker='x',color=ORANGE,s=20,linewidths=1)
    z.set_yticks([]);z.spines['left'].set_visible(False);z.spines['bottom'].set_color('#AAAAAA')
    z.set_xlim(.3,20.7);z.set_xticks([1,5,10,15,20]);z.set_xlabel('Candidate rank')
    if col==0:z.set_ylabel('k = 0',rotation=0,rotation_mode='anchor',labelpad=18,fontsize=7.5,va='center')
    audit_panels.extend([{'panel':chr(97+col),'run':run,'quantity':'Mean-score empirical tail','background_pairs':4005,'uncertainty':'No CI; deterministic empirical function of shared finite background'},
                         {'panel':chr(100+col),'run':run,'quantity':'All frozen Top20 mean-score tails','rows':20,'zero_strip_count':int((k==0).sum()),'low_nonzero_count':int(((k>0)&(k<20)).sum()),'uncertainty':'Tail counts encoded; no iid error bars; zero strip is nonnumeric'}])
...
legend1=fig.legend(handles=top_handles,loc='upper center',bbox_to_anchor=(.53,.987),ncol=4,columnspacing=1.1,handlelength=1.6)
bottom_handles=[Line2D([],[],color=TEAL,marker='o',mfc='white',ls='none',ms=4,label='1–19 exceedances'),Line2D([],[],color=TEAL,marker='o',ls='none',ms=4,label='20 or more'),Line2D([],[],color=ORANGE,marker='x',ls='none',ms=4,label='Zero: unresolved (strip)')]
legend2=fig.legend(handles=bottom_handles,loc='lower center',bbox_to_anchor=(.53,.015),ncol=3,columnspacing=1.5,handlelength=1.2)
fig.canvas.draw()
renderer=fig.canvas.get_renderer()
assert legend1.get_window_extent(renderer).y0>max(a.get_tightbbox(renderer).y1 for a in upper)
assert legend2.get_window_extent(renderer).y1<min(a.get_tightbbox(renderer).y0 for a in strips)
fig.savefig(OUT/'Fig4_FPP_review.pdf')
fig.savefig(OUT/'Fig4_FPP_review.svg')
fig.savefig(OUT/'Fig4_FPP_review.png',dpi=600)
fig.savefig(OUT/'Fig4_FPP_review.tiff',dpi=600,pil_kwargs={'compression':'tiff_lzw'})
plt.close(fig)
    ...
    for run in RUNS:
        ...
        for start in [0,10]:
            ...
            for (row,col),cell in table.get_celld().items():
                ...
                if row>0 and col==7 and cell.get_text().get_text()=='Unresolved':cell.get_text().set_color(ORANGE)
            fig.text(.029,.949,f'{run} candidate pairs | Ranks {start+1}–{start+10}',fontsize=14,fontweight='bold')
            fig.text(.029,.915,'Frozen C ranking with GWLR-FPP-01 background comparison',fontsize=10,color='#555555')
            notes=['S and weighted contributions are model means; candidate order remains the frozen consensus ranking.',
                   'TriLens conditional FPP = k/4005. Unresolved: k = 0, not zero risk. It is not a lensing probability or annual FAR.',
...
pdfchecks=[]
for p in [OUT/'Fig4_FPP_review.pdf',TAB/'Table_efficiency_FPP1_review.pdf',TAB/'Tables_candidates_Top20_review.pdf']:
...
qa={'inputs':sources,'panels':audit_panels,'pdf_checks':pdfchecks,'candidate_table_pages':pages,'backend':'Python','plot_size_mm':[183,155],'rank_changes':False,'paper_pushed':False,'zero_policy':'Separate nonnumeric strip; no upper limit assigned','all_60_candidates_displayed':True}
(BASE/'FIGURE_QA.json').write_text(json.dumps(qa,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'figure':str(OUT/'Fig4_FPP_review.png'),'pdf_pages':len(pdfchecks),'source_hashes':len(sources),'table_pages':len(pages)},ensure_ascii=False))
