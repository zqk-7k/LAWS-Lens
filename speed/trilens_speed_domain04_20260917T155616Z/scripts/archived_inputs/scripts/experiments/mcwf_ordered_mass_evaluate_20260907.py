...
import mcwf_ordered_mass_predictor_20260907 as mass_model
...
def calibrate(root, dep, ms):
    path = root / f'ordered_mass_predictor/calibration/{dep}/seed_{ms}/CALIBRATION.json'
    if path.exists():
...
def run(root, arm):
    trial = root / f'trials/ORDERED-MASS-CONV-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'arm': arm, 'model': '253 orderedMc template responses,27channels,localmass-axisCNN;12288sourceparents,160noiseblocks',
        ...
        'OOD': 'no new positive reward outside calibration or incompatible mass; posterior endpoint mass>25percent makes new terms neutral;this is a boundary-occupancy proxy,not an estimated probability outside5-200Msun',
