...
def main():
    ...
    fixtures = R/'fixtures/inference'
    output = R/'verification/all_components'
    ...
    for run in ('O3', 'O4a', 'O4b'):
        ...
        if a.baseline:
            ...
            e.iloc[selected].reset_index(drop=True).to_parquet(fixture/'events.parquet', index=False)
            for src, dest in [('inference_inputs/validation/raw2s.npy', 'raw2s.npy'),
                ...
                np.save(fixture/dest, np.load(old/src, mmap_mode='r')[selected])
            ...
            for kind in ('short', 'rnc', 'ordered', 'multirate', 'joint'):
                ...
                with np.load(got) as g, np.load(expected) as ref:
                    for key in ref.files:
                        delta = float(np.max(np.abs(g[key].astype(float)-ref[key])))
                        errors[key] = delta
                        np.testing.assert_allclose(g[key], ref[key], rtol=3e-5, atol=3e-6)
                records.append(dict(run=run, seed=seed, component=kind, events=len(events),
                                    status='PASS', maximum_absolute_errors=errors))
    ...
    (output/'REPORT.json').write_text(json.dumps(dict(status='PASS', comparisons=records,
        components=45, events_per_run=4, all_catalog_inference=False,
