#!/usr/bin/env python3
"""Shared, leakage-controlled utilities for the v6 real-noise deployment.
...
v3 = module_from(REPO / "scripts/experiments/20_real_noise_injection_v3_physical.py", "v3_for_v6_common")
pilot = module_from(REPO / "scripts/real_search/37_unified_intrinsic_multitask_pilot.py", "pilot_for_v6_common")
specmod = module_from(REPO / "scripts/experiments/26_spectrogram_encoder_pilot.py", "spec_for_v6_common")
...
def load_unified_model(checkpoint: Path) -> tuple[torch.nn.Module, dict[str, Any]]:
    ...
    model = pilot.PhysicsRegularizedEncoder(
        specmod.TriggerAlignedDualScaleSpectrogramEncoder(base_channels=24)
    )
...
def build_mixed_pair_table(
    ...
    output = seed_dir / "results" / f"mixed_{split}_pair_features_v6.parquet"
    if output.exists():
    ...
    events.to_parquet(result_dir / f"mixed_{split}_synthetic_events_v6.parquet", index=False)
    np.save(result_dir / f"mixed_{split}_synthetic_sky_posteriors_nside{v3.SKY_NSIDE}_v6.npy", maps)
    np.save(result_dir / f"mixed_{split}_unified_embeddings_v6.npy", embedding)
    np.save(result_dir / f"mixed_{split}_intrinsic_predictions_standardized_v6.npy", prediction)
    ...
    write_json(
        result_dir / f"mixed_{split}_catalog_audit_v6.json",
        {
...
def embed_real_events_unified(
    ...
    emb_path = feature_dir / "real_waveform_embeddings_unified_v6.parquet"
    pair_path = feature_dir / "real_waveform_similarity_unified_v6.parquet"
    audit_path = seed_dir / "results/real_waveform_deployment_audit_unified_v6.json"
...
def _rank_real_pairs(frame: pd.DataFrame, weights: dict[str, float], method: str) -> pd.DataFrame:
    ...
    # All v6 channels are globally calibrated, symmetric pair evidence. There
...
def score_real_catalog(
    ...
    for method, weights in methods.items():
        ...
        all_catalog.to_parquet(result_dir / f"real_pair_scores_all_catalog_{method}_v6.parquet", index=False)
        strict.to_parquet(result_dir / f"real_pair_scores_strict_h1l1_bbh_{method}_v6.parquet", index=False)
        all_catalog.head(100).to_csv(result_dir / f"candidate_shortlist_all_catalog_{method}_v6.csv", index=False)
        strict.head(100).to_csv(result_dir / f"candidate_shortlist_strict_h1l1_bbh_{method}_v6.csv", index=False)
