#!/usr/bin/env python3
"""Formal mixed-population O3/O4a real-noise deployment (v6).
...
from scripts.real_search.unified_v6_common import (
    FAMILIES,
...
DEFAULT_DEVELOPMENT_SELECTION = (
    DEFAULT_OUT / "development/pilot_selection_final/pilot_selection_validation_only.json"
)
...
v3 = module_from(REPO / "scripts/experiments/20_real_noise_injection_v3_physical.py", "v3_for_v6")
v5 = module_from(REPO / "scripts/real_search/35_real_noise_injection_v5_physical_source.py", "v5_for_v6")
...
def ensure_training_material(seed_dir: Path, source_bank: Path, seed: int, samples: int) -> None:
    for family in FAMILIES:
        ...
        run_checked(
            ...
            seed_dir / "logs" / f"materialize_multinoise_{family.lower()}_v6.log",
        )
...
def train_unified(
    ...
    out = seed_dir / "waveform_gate" / f"unified_full24_aux_{tag}_q_{q_tag}_v6"
    ...
    run_checked(
        ...
        seed_dir / "logs" / "train_unified_v6.log",
    )
    return checkpoint
...
def audit_time_calibration(shared: Path) -> None:
    output = shared / "time_delay_bandwidth_sensitivity_v6.json"
    if output.exists():
        return
...
def run_seed(
    ...
    complete = seed_dir / "seed_summary_v6.json"
    if complete.exists():
    ...
    write_json(
        shared / "time_delay_prior_source_audit_v6.json",
        {
    ...
    write_json(seed_dir / "results/split_integrity_audit_v6.json", split_audit)
    ...
    waveform_grid.to_csv(seed_dir / "results/waveform_intrinsic_grid_validation_v6.csv", index=False)
    write_json(seed_dir / "results/waveform_channel_calibration_v6.json", waveform_config)
    validation.to_parquet(seed_dir / "results/fusion_validation_pairs_v6.parquet", index=False)
    ...
    for name, grid in {
        "fusion_weight_grid_retrieval_unconstrained_v6.csv": grid_retrieval_free,
        "fusion_weight_grid_retrieval_strictly_positive_v6.csv": grid_retrieval_positive,
        "fusion_weight_grid_candidate_unconstrained_v6.csv": grid_candidate_free,
        "fusion_weight_grid_candidate_strictly_positive_v6.csv": grid_candidate_positive,
        "fusion_weight_grid_time_sky_retrieval_v6.csv": grid_time_sky_retrieval,
        "fusion_weight_grid_time_sky_candidate_v6.csv": grid_time_sky_candidate,
    }.items():
        grid.to_csv(seed_dir / "results" / name, index=False)
    gate = waveform_gate(validation, candidate_free, time_sky_candidate, seed=seed)
    write_json(seed_dir / "results/waveform_gate_validation_v6.json", gate)
    weights = {
    ...
    write_json(seed_dir / "results/selected_weights_v6.json", weights)
    ...
    test.to_parquet(seed_dir / "results/fusion_heldout_test_pairs_v6.parquet", index=False)
    methods = {
    ...
    query.to_parquet(seed_dir / "results/heldout_test_query_ranks_v6.parquet", index=False)
    retrieval.to_csv(seed_dir / "results/heldout_test_retrieval_metrics_v6.csv", index=False)
    pair.to_csv(seed_dir / "results/heldout_test_pair_level_metrics_v6.csv", index=False)
    bootstrap = bootstrap_system_ci(query, draws=bootstrap_draws, seed=seed + 4000)
    bootstrap.to_csv(seed_dir / "results/heldout_test_bootstrap_95ci_v6.csv", index=False)

    validation_embedding = np.load(seed_dir / "results/mixed_val_unified_embeddings_v6.npy")
    real_methods = {
        "retrieval_three_channel_unconstrained": retrieval_free,
        "retrieval_three_channel_strictly_positive": retrieval_positive,
        "candidate_three_channel_unconstrained": candidate_free,
    ...
    for scope in ("strict", "all"):
        ...
        source.to_parquet(result_dir / f"real_pair_scores_{scope}_primary_v6.parquet", index=False)
        source.head(100).to_csv(result_dir / f"candidate_shortlist_{scope}_primary_v6.csv", index=False)
...
def aggregate(out_root: Path, deployment: str, seeds: list[int]) -> None:
    ...
    query = pd.concat(
        [pd.read_parquet(root / f"seed_{seed}/results/heldout_test_query_ranks_v6.parquet") for seed in seeds],
        ignore_index=True,
    )
    retrieval = pd.concat(
        [pd.read_csv(root / f"seed_{seed}/results/heldout_test_retrieval_metrics_v6.csv") for seed in seeds],
        ignore_index=True,
    )
    pair = pd.concat(
        [pd.read_csv(root / f"seed_{seed}/results/heldout_test_pair_level_metrics_v6.csv") for seed in seeds],
        ignore_index=True,
    )
    bootstrap = pd.concat(
        [pd.read_csv(root / f"seed_{seed}/results/heldout_test_bootstrap_95ci_v6.csv") for seed in seeds],
        ignore_index=True,
    )
    query.to_parquet(root / "heldout_test_query_ranks_all_seeds_v6.parquet", index=False)
    retrieval.to_csv(root / "heldout_test_retrieval_metrics_per_seed_v6.csv", index=False)
    across_seed_summary(retrieval).to_csv(root / "heldout_test_retrieval_metrics_across_seed_v6.csv", index=False)
    pair.to_csv(root / "heldout_test_pair_level_metrics_per_seed_v6.csv", index=False)
    pair_for_summary = pair.copy()
    pair_for_summary["subset"] = "overall"
    across_seed_summary(pair_for_summary).to_csv(
        root / "heldout_test_pair_level_metrics_across_seed_v6.csv", index=False
    )
    bootstrap.to_csv(root / "heldout_test_bootstrap_95ci_per_seed_v6.csv", index=False)
    summaries = [json.loads((root / f"seed_{seed}/seed_summary_v6.json").read_text(encoding="utf-8")) for seed in seeds]
    write_json(
        root / "deployment_summary_v6.json",
        {
            "deployment": deployment,
            "seeds": seeds,
...
def main() -> None:
    ...
    config = {
        "protocol": "real_noise_injection_v6_unified_physics",
        "status": "running",
    ...
    write_json(args.out_root / "run_config_v6.json", config)
