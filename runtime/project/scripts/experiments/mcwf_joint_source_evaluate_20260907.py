...
def data(root,dep,ms,es,split):
    if split=='development':
        path=root/f'joint_source_density/pair_features/{dep}/seed_{es}/development.npz'
        f=pd.read_parquet(root/f'expanded_calibration/development/{dep}/seed_{es}/pairs.parquet')
        a=np.load(root/f'joint_source_density/models/{dep}/seed_{ms}/validation_predictions.npz')
    else:
        path=root/f'joint_source_density/pair_features/{dep}/seed_{es}/{split}.npz'
...
def fit(root,dep,ms,es,kind):
    path=root/f'joint_source_density/calibration/{kind}/{dep}/seed_{es}/fit.json'
...
def run(root,kind):
    trial=root/'trials'/f'JOINT-SOURCE-CONDITIONAL-{kind.upper()}'
    ...
    dev.json_write(trial/'contracts/RECIPE.json',{'created_utc':datetime.now(timezone.utc).isoformat(),
        'same_kind_O3_O4a':kind,'new_model':'retrainedraw2s+phase sourceencoderwithjoint(logMc,logitq,atanhchieff)mixturedensityhead',
    ...
    print(json.dumps({'joint_source_selection':kind,'states':states}),flush=True)
    rows,guards=[],[]
    for dep in e.DEPS:
        real={}
        for ms,es in zip(body.MODEL_SEEDS,dev.SEEDS):
