...
OUT_ROOT = Path('runs/pm_mass_1e4_1e10_td_min24s_ep50_aux_compare_20260608')
DATA_ROOTS = {
    'ET': Path('data_generation/pm_mass_1e4_1e10_td_min24s_matchroots/ET'),
    'LIGO': Path('data_generation/pm_mass_1e4_1e10_td_min24s_matchroots/LIGO'),
}
...
def make_cfg(detector: str, mode: str) -> MatchRunConfig:
    ...
    return MatchRunConfig(
        ...
        model_type='PM',
        data_mode=mode,
        out_dir=OUT_ROOT / f'{detector.lower()}_{mode}_pm_ep50',
...
def split_pack(arrays, splits, cfg: MatchRunConfig, detector: str, split: str, model=None, out_dir: Path | None = None):
    ...
    raw_obs = aux.catalog_observable_frame(cfg.data_root, 'PM', splits['lensed'][split], splits['unlensed'][split]).reset_index(drop=True)
    time_obs = catalog_trigger_time_frame(cfg.data_root, 'PM', splits['lensed'][split], splits['unlensed'][split], detector=detector).reset_index(drop=True)
    emb = None
    scores = None
