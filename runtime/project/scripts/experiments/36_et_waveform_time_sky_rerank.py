...
def load_split(family: str, split: str, out_dir: Path):
    run_dir = RUNS[family]
    ...
    obs = aux.perturb_observables(obs, MODE, seed=51000 + abs(hash((family, split, MODE))) % 10000).reset_index(drop=True)
    cache = out_dir / f'{family}_{split}_scores.npy'
    if cache.exists():
...
def train_examples(obs: pd.DataFrame, gt: np.ndarray, scores: np.ndarray, ranks: np.ndarray, family: str):
    rng = np.random.default_rng(62000 + abs(hash(family)) % 10000)
...
def eval_full(clf, obs: pd.DataFrame, gt: np.ndarray, scores: np.ndarray, ranks: np.ndarray):
    ...
    return {
        'r@1': float(np.mean(r <= 1)), 'r@5': float(np.mean(r <= 5)), 'r@10': float(np.mean(r <= 10)),
        'r@50': float(np.mean(r <= 50)), 'r@100': float(np.mean(r <= 100)), 'r@500': float(np.mean(r <= 500)),
        'median_true_rank': float(np.median(r)), 'valid': int(len(valid))
    }


def run_family(family: str, out_root: Path):
    val_obs, val_gt, val_scores = load_split(family, 'val', out_root / family / 'val')
    test_obs, test_gt, test_scores = load_split(family, 'test', out_root / family / 'test')
    val_ranks = row_ranks(val_scores); test_ranks = row_ranks(test_scores)
    Xv, yv = train_examples(val_obs, val_gt, val_scores, val_ranks, family)
    ...
    result = {'family': family, 'mode': MODE, 'method': 'et_waveform_model_delta_time_sky_sep_catalog_rerank_hgb', 'features': ['log1p_delta_time', 'sky_sep', 'waveform_score', 'waveform_reciprocal_rank'], 'neg_per_pos': NEG_PER_POS, 'train_examples': int(len(yv)), 'train_positive': int(yv.sum()), 'val_auc_sampled': float(roc_auc_score(yv, pv)), **eval_full(clf, test_obs, test_gt, test_scores, test_ranks)}
    print(result, flush=True)
    return result


def main():
    out_root = Path('runs/et_noisy_waveform_time_sky_rerank_20260601')
