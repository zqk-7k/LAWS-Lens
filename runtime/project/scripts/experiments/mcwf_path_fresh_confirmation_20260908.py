...
def unit(root):
    ...
    for dep in t.DEPS:
        for slot, seed in zip(t.MODEL_SLOTS, t.SEEDS):
            x = np.concatenate([t.old_features(dep, seed, 'validation')[:8], np.load(LOW/f'features/{dep}/{seed}_validation.npy')[:8]], 1)
            ...
            errors = {k: float(abs(current[k]-olda[k][:8]).max()) for k in ('p', 'joint', 'outside')}
