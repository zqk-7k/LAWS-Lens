...
def prepare(root):
    ...
    contract = {
        ...
        'source_distribution': 'reserved global GW-LMC IDs as3000plan with documented pre-inference calendar feasibility replacements;2.5PLUS BBH Any_Detected_SNR1;proposal ratio<=4 selection inherited and disclosed',
        'calendar_feasibility_repair':'replace training-only reserved systems with delay>=10Julian years by hash-ordered unused same-family systems with delay<T;no validation/test replacement;before any fit or scores',
    ...
    replacements=[]
    for rec in reserved:
        if not rec['images'] or 0<=rec['images']['delay_days']<3652.5:continue
        if rec['split']!='train':raise RuntimeError('Calendar feasibility would alter outer heldout split;requires separate protocol')
        original=dict(rec)
        candidates=table[table.lens_is_subhalo==(rec['family']=='subhalo')]
        for r in sorted(candidates.to_dict('records'),key=lambda x:stable('ETNSO-calendar-repair-20260915',int(x['event_id']))):
            uid=int(r['event_id']);im=src.choose_images(pd.Series(r))
            if uid in seen or uid in pilot_ids or im is None or im['proposal_snr_ratio']>4 or not 0<=im['delay_days']<3652.5:continue
            rec.update(source_id=uid,gwlmc_row=int(r['gwlmc_row']),images=im);seen.add(uid)
            replacements.append({'old':original,'new':dict(rec),'reason':'both images cannot fall within10year exposure'})
            break
        else:raise RuntimeError('No disjoint calendar-feasible replacement')
    write(root/'contracts/CALENDAR_FEASIBILITY_REPLACEMENTS.json',replacements)
    ...
    for family,subhalo in [('smooth',False),('subhalo',True),('unlensed',None)]:
        ...
        for r in ordered:
            ...
            if subhalo is not None and (images is None or images['proposal_snr_ratio']>4 or not 0<=images['delay_days']<3652.5):continue
