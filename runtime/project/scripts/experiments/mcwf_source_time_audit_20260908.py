        ...
        for k, i in enumerate(ids):
            for j in ids[k+1:]:
                ...
                if dt > 0 and np.isfinite(dt):
                    rows.append({'global_source_group_id': f'GW-LMC-source:{r.event_id}', 'source_id': int(r.event_id),
...
def initialize(root):
    ...
    dev.json_write(root / 'contracts/ANALYSIS_CONTRACT.json', {
        'id': 'MCWF-SOURCE-GROUP-TIME-11', 'UTC': datetime.now(timezone.utc).isoformat(), 'status': t.STATUS,
        ...
        'source_unit': 'One total weight per independent source,uniform choice of detectable pair within source;not uniform all image-pairs. Remove union of current validation/test global source IDs across both runs and all seeds.',
    ...
    original = t.protected() + [{'path': str(p), 'sha256': dev.sha(p), 'bytes': p.stat().st_size} for p in (SOURCE, MEMBERSHIP)]
    for dep in t.DEPS:
    ...
    shutil.copy2(__file__, root / 'scripts/source_time_audit.py')
    ...
    for dep in t.DEPS:
        ...
        rows.append({'deployment': dep, 'original_raw_pairs': len(raw), 'original_independent_sources': raw.global_source_group_id.nunique(),
                     ...
                     'shared_old_prior_sources_with_current_validation_test': True})
        for kind in KINDS:
            for seed in t.SEEDS:
