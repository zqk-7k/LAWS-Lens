...
def run(root,kind,mode):
    code=f'HIGHER-MODE-COMPLETE-GRID-{kind.upper()}-{mode.upper()}'
    ...
    dev.json_write(trial/'contracts/RECIPE.json',{'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'simulation_plateau':'baseline+all validation-eligible points per power passing all frozen reused-test guards;completeeligiblegridwithouttop4truncation',
    ...
    for dep in e.DEPS:
        ...
        for ms,es in zip(body.MODEL_SEEDS,dev.SEEDS):
            ...
            for power in POWERS:pool.extend([c for c in eligible if c['spec']['power']==power and c['spec']['beta']>0])
