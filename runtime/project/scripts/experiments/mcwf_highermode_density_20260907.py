...
SEEDS = (202609181, 202609182, 202609183)
K, D, EPOCHS = 4, 3, 60


class Density(nn.Module):
    def __init__(self, dim=1728+6*len(higher.PARAMETERS)):
        super().__init__()
...
def initialize(root):
    ...
    dev.json_write(path, {'created_utc': datetime.now(timezone.utc).isoformat(), 'same_O3_O4a': True,
        ...
        'input': 'H1L1 peak2s4096points physical40-580Hz; event-PSD phase-quadrature log1p power from576frozenDplus4608XPHMpolarizationtemplates; no metadata as network inputs',
        'network': '15552->256 LayerNorm SiLU dropout0.1->128 SiLU->4 full-covariance3D Gaussian components',
        ...
        'references': ['https://www.microsoft.com/en-us/research/wp-content/uploads/2016/02/bishop-ncrg-94-004.pdf',
            'https://docs.pytorch.org/docs/stable/distributions.html', 'https://arxiv.org/abs/2004.06503'],
...
def inputs(root, dep, split):
    ...
    b = higher.development(root, dep, split)
...
def train(root, dep, ms, seed):
    initialize(root)
    out = root / f'highermode_density/models/{dep}/seed_{ms}'
    ...
    for epoch in range(first, EPOCHS+1):
        ...
        if epoch % 10 == 0:
            print(json.dumps({'highermode_training': dep, 'seed': seed, **history[-1]}), flush=True)
    ...
    print(json.dumps({'highermode_training_done':report}),flush=True)


def prediction(root,dep,ms,es,split):
    cp=root/f'highermode_density/models/{dep}/seed_{ms}/validation_selected_model.pt'
    if not (cp.parent/'COMPLETE.json').exists():
        raise RuntimeError('Higher-mode density training incomplete')
    path=root/f'highermode_density/predictions/{dep}/model_{ms}_eval_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
    ...
    coarse=np.load(feature_path); extra=higher.deployment(root,dep,es,split)
...
if __name__=='__main__':
    ...
    initialize(a.root);dev.json_write(a.root/'highermode_density/contracts/UNIT_TESTS.json',tests())
    for dep in e.DEPS:
