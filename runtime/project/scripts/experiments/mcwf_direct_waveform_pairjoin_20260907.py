...
def cached(root, dep, es, split):
    ...
    if path.exists():
        a = np.load(path)
        # Real pair tables are seed-ranked; reuse symmetric features by identity.
        source = pd.MultiIndex.from_arrays([np.minimum(a['idx_i'], a['idx_j']), np.maximum(a['idx_i'], a['idx_j'])])
        target = pd.MultiIndex.from_arrays([np.minimum(f.idx_i, f.idx_j), np.maximum(f.idx_i, f.idx_j)])
        if not source.is_unique or not target.is_unique or len(source) != len(target):
            raise RuntimeError('Invalid cached pair identities')
        order = source.get_indexer(target)
        if (order < 0).any():
            raise RuntimeError('Pair scope changed')
        return f, a['features'][order]
    x = compare(raw, i, j)
...
def run(root):
    trial = root/'trials/DIRECT-PHASE-MATCH-PAIRJOIN'
    if trial.exists():
    ...
    dev.json_write(trial/'contracts/RECIPE.json', {
        ...
        'implementation_fix': 'reuse real pair features by unordered event identity, not seed-dependent row ordering; initial aborted trial retained',
