...
OUT_ROOT = Path('runs/ligo_grid_skymap_predictor_rerank_20260604')
...
class SkyMapCNN(nn.Module):
    def __init__(self, in_channels: int = 2, n_pix: int = N_PIX) -> None:
        ...
        self.net = nn.Sequential(
            nn.Conv1d(in_channels, 32, 31, stride=4, padding=15, bias=False), nn.BatchNorm1d(32), nn.GELU(),
            nn.Conv1d(32, 64, 21, stride=4, padding=10, bias=False), nn.BatchNorm1d(64), nn.GELU(),
            nn.Conv1d(64, 128, 15, stride=4, padding=7, bias=False), nn.BatchNorm1d(128), nn.GELU(),
            nn.Conv1d(128, 128, 9, stride=2, padding=4, bias=False), nn.BatchNorm1d(128), nn.GELU(),
            nn.Conv1d(128, 192, 7, stride=2, padding=3, bias=False), nn.BatchNorm1d(192), nn.GELU(),
        )
        self.attn = nn.Sequential(nn.Conv1d(192, 48, 1), nn.GELU(), nn.Conv1d(48, 1, 1))
        self.head = nn.Sequential(nn.Linear(192 * 3, 384), nn.GELU(), nn.Dropout(0.15), nn.Linear(384, n_pix))
...
def train_model(job, train_ds, train_raw, val_ds, val_raw, out_dir):
    ...
    best = {'val_kl': 999.0, 'state': None, 'epoch': 0}
    history = []
    for epoch in range(1, EPOCHS + 1):
        ...
        if kl < best['val_kl']:
            best = {'val_kl': kl, 'state': {k: v.detach().cpu() for k, v in model.state_dict().items()}, 'epoch': epoch}
...
def run_job(job):
    ...
    row = {
        ...
        'method': 'ligo_grid_skymap_predictor_rerank',
        'features': 'log1p_delta_time_obs,predicted_grid_skymap_overlap,waveform_score,waveform_reciprocal_rank',
