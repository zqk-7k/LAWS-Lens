    ...
    for dep in n.DEPS:
        ...
        for left, right in (('bin512', '16'), ('2', '4'), ('4', '8'), ('8', '16')):
            ...
            for name, mask in (('all', np.ones(len(i), bool)), ('true_companion', truth),
                               ('null', ~truth), ('profile_involved', active[i] | active[j])):
                values = delta[mask]
                rows.append({'deployment': dep, 'left': left, 'right': right, 'stratum': name,
                    ...
                    'max_norm_error_right': norms[right],
                    'converged': bool(np.quantile(values, .99)<=1e-4 and values.max()<=1e-3 and norms[right]<=1e-6)})
