...
import mcwf_detector_reliability_pool_20260907 as mass_model
...
def run(root, arm):
    trial = root / f'trials/DETECTOR-RELIABILITY-POOL-{mass_model.MODE.upper()}-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'arm': arm, 'pooling': mass_model.MODE, 'model': 'development-CE-calibrated detector-information-weighted H1/L1 single-detector mass prediction pool;same2sinput;notfullnetworkPE',
