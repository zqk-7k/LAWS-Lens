...
dev = e.dev
COUNTS = {'train': 8192, 'validation': 0}
VIEWS = {'train': 4, 'validation': 1}
BLOCKS = {'train': 64, 'validation': 0}
DATA_SEEDS = {'train': 202609270, 'validation': 202609280}
CTX = {}
...
def initialize(root):
    ...
    dev.json_write(path, {
        ...
        'purpose': 'additional8192sourceparentsand64newnoiseblocks/run; train alongside existing4096sources; existing512source developmentvalidation remains unchanged',
        ...
        'statistics': '8192 additionalsourceunits,65536trainingviews,64newnoiseblocks; all distinct from the preceding expanded4096/512sources and128noiseblocks',
...
def noise(root, dep):
    ...
    original=root.parent/f'expanded_data/{dep}/noise/noise_manifest.csv'
    if not original.exists():
        raise RuntimeError('Missing parent development noise manifest')
    for r in pd.read_csv(original).itertuples():
        exclusions.append({'start':r.start_gps,'end':r.end_gps,'origin':'preceding_expanded_train_and_validation'})
    ...
    for k in range(total):
        ...
        chosen, rejected = data.select_valid_noise_references(pd.DataFrame(parts), 1,
            np.random.default_rng(data.stable_seed(dep, 'additional-waveform-noise', 202609270, k)),
