...
def generate(root):
    ...
    for row in rows:
        ...
        raw_snr = float(phys.optimal_network_snr(clean, frequency, psds))
        if not np.isfinite(raw_snr) or raw_snr <= 0:
            raise RuntimeError('Invalid unscaled optimal SNR')
...
def extract_one(root, idx):
    ...
    with np.load(data_path) as a:
        strain = np.asarray(a['noisy_padded'][:, FS:FS+N], dtype=np.float64)
        clean = np.asarray(a['clean'], dtype=np.float64)
    ...
    for channel, ifo in enumerate(ifos):
        ...
        def compute(v):
            return np.asarray(matched_filter(ts, FrequencySeries(np.asarray(v, dtype=np.complex128), delta_f=DF), psd=p,
