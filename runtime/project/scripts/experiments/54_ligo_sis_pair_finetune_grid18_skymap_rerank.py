...
OUT_ROOT = Path('runs/ligo_sis_pair_finetune_grid18_skymap_rerank_20260605')
...
PRETRAINED_CKPT = Path("runs/ligo_sis_resnet_grid18_skymap_rerank_20260604/ligo_noisy_sis/grid_skymap_cnn.pt")
...
class PairSkyMapDataset(Dataset):
    def __init__(self, ds: EvaluationSet, raw_obs: pd.DataFrame, gt: np.ndarray, seed: int = 20260605) -> None:
        self.ds = ds
        self.y = soft_skymap(raw_obs)
        self.gt = gt.astype(np.int64)
        self.valid = np.flatnonzero(self.gt >= 0).astype(np.int64)
        self.n = len(ds)
        self.rng = np.random.default_rng(seed)

    def __len__(self) -> int:
        return len(self.valid) * NEGATIVES_PER_ANCHOR

    def __getitem__(self, item: int):
        anchor = int(self.valid[item % len(self.valid)])
        pos = int(self.gt[anchor])
        neg = int(self.rng.integers(0, self.n))
        while neg == anchor or neg == pos or self.gt[anchor] == neg:
            neg = int(self.rng.integers(0, self.n))
        return (
            self.ds[anchor], torch.from_numpy(self.y[anchor]),
            self.ds[pos], torch.from_numpy(self.y[pos]),
            self.ds[neg], torch.from_numpy(self.y[neg]),
        )
...
def train_model(job, train_ds, train_raw, train_gt, val_ds, val_raw, out_dir):
    ...
    train_loader = DataLoader(PairSkyMapDataset(train_ds, train_raw, train_gt), batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
    ...
    if PRETRAINED_CKPT.exists():
        ...
        print('LOAD_PRETRAINED', PRETRAINED_CKPT, flush=True)
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    sched = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=EPOCHS)
    best = {'val_kl': 999.0, 'val_mean_err': 999.0, 'state': None, 'epoch': 0}
    history = []
    for epoch in range(1, EPOCHS + 1):
        model.train(); losses = []; kl_losses = []; pair_losses = []
        for xa, ya, xp, yp, xn, yn in train_loader:
            xa = xa.to(device); ya = ya.to(device)
            xp = xp.to(device); yp = yp.to(device)
            xn = xn.to(device); yn = yn.to(device)
            la = model(xa); lp = model(xp); ln = model(xn)
            logpa = F.log_softmax(la, dim=1)
            logpp = F.log_softmax(lp, dim=1)
            logpn = F.log_softmax(ln, dim=1)
            loss_kl = (
                F.kl_div(logpa, ya, reduction='batchmean')
                + F.kl_div(logpp, yp, reduction='batchmean')
                + 0.5 * F.kl_div(logpn, yn, reduction='batchmean')
            ) / 2.5
            pa = torch.softmax(la, dim=1)
            pp = torch.softmax(lp, dim=1)
            pn = torch.softmax(ln, dim=1)
            log_pos = torch.log(torch.sum(pa * pp, dim=1) + EPS)
            log_neg = torch.log(torch.sum(pa * pn, dim=1) + EPS)
            target = torch.ones_like(log_pos)
            loss_pair = F.margin_ranking_loss(log_pos, log_neg, target, margin=PAIR_MARGIN)
            loss = loss_kl + PAIR_LAMBDA * loss_pair
        ...
        row = {
            ...
            'loss_kl': float(np.mean(kl_losses)),
            'loss_pair': float(np.mean(pair_losses)),
            'val_kl': kl,
            'val_mean_err': float(err.mean()),
            'val_median_err': float(np.median(err)),
            'val_lt1': float(np.mean(err < 1.0)),
        }
        history.append(row)
        print('PAIR_GRID_EPOCH', job['family'], row, flush=True)
...
def run_job(job):
    ...
    _, train_ds, train_raw, _, train_gt, _ = load_pack(job, 'train', out_dir / 'train', False)
    ...
    row = {
        ...
        'method': 'ligo_sis_pair_finetune_grid18_skymap_rerank',
        'features': 'log1p_delta_time_obs,pair_finetune_grid18_skymap_overlap,waveform_score,waveform_reciprocal_rank',
        'sky_model': f'SkyMapCNN_pair_finetune_grid_{DEC_BINS}x{RA_BINS}',
