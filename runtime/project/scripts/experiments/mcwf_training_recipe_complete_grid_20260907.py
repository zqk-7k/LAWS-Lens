...
import mcwf_training_recipe_ensemble_evaluate_20260907 as rnc

module = SimpleNamespace(GAMMAS=rnc.GAMMAS, BETAS=rnc.BETAS,
    values=rnc.values, calibrate=rnc.calibration,
    score=lambda f,x,spec,arm: rnc.score(f,x,spec)[:3])
...
def run(root, model, arm):
    code = 'TRAINING-RECIPE-ENSEMBLE-COMPLETE-GRID-MASS-COSINE'
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'formula': 'Z_FRT+gamma*newRNCfiniteMctail+beta*boundednewRNCcosineLR;identical to TRAINING-RECIPE-ENSEMBLE-MASS-COSINE',
...
if __name__ == '__main__':
    ...
    run(args.root, 'training_recipe_ensemble', 'MASS-COSINE')
