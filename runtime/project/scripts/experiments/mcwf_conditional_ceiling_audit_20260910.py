#!/usr/bin/env python3
"""Independent requirement-by-requirement audit of frozen R69 outputs."""
import os
...
def main(root):
    ...
    for file in sorted(files):
        ...
        for method, historical in [(app.METHODS[0], 'NODUP-DIRECT-REPLAY'),
                                   (app.METHODS[1], 'NODUP-CONDITIONAL-DEFICIT')]:
        ...
        for method in app.METHODS:
            ...
            if method == app.NEW:
                ...
                expected[active], outside, clipped = app.pilot.apply(expected[active],
                    ...
                    f.shared_profile_deficit.to_numpy(float)[active], cfg['ceiling_model'])[:3]
                if not np.array_equal(expected, f.waveform_score.to_numpy(float)):
                    raise RuntimeError('Candidate score differs from frozen R68 formula')
                ...
                for position in np.flatnonzero(changed):
                    deltas.append({**f.iloc[position][columns].to_dict(), 'deployment': dep,
                        ...
                        'R67_waveform': float(frames[app.METHODS[1]].waveform_score.iloc[position])})
            if panel == 'real':
    ...
    shutil.copy2(__file__, root / 'scripts/conditional_ceiling_audit.py')
