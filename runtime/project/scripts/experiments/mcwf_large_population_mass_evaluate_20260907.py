...
import mcwf_large_population_mass_only_20260907 as mass_model
...
def calibrate(root, dep, ms):
    path = root / f'large_population_mass_only/calibration/{dep}/seed_{ms}/CALIBRATION.json'
    if path.exists():
...
def run(root, arm):
    trial = root / f'trials/LARGE-POPULATION-MC-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'arm': arm, 'model': 'unchangedfineMc15855featuremass-onlyMDN; train12288sourceparentsand160noiseblocks vs4096/96',
