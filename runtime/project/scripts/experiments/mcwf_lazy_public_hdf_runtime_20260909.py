...
class LazyStrain:
    ...
    def __getitem__(self,index):
        return np.asarray(self.file['strain/Strain'][index],np.float32)
...
def pilot():
    out=ROOT/'audit/LAZY_HDF_READER_PILOT.json'
    if out.exists():
    ...
    for detector,index in (('H1',0),('L1',1)):
        ...
        rows.append({'detector':detector,'pass':True,'metadata_and_DQ_exact':True,
            'accepted256s_reference_exact':True,'extra_hyperslabs_exact':True,
...
def generate(workers):
    check=json.loads((ROOT/'audit/LAZY_HDF_READER_PILOT.json').read_text())
    ...
    if not contract.exists():
        n.write_json(contract,{'UTC':n.utc(),'transport_only':True,'pilot_sha256':n.sha(ROOT/'audit/LAZY_HDF_READER_PILOT.json'),
