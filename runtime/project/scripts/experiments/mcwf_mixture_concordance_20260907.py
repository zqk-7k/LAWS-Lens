...
import mcwf_mixture_evaluate_20260907 as mix
...
def score(f,x,spec):
    ...
    gate=np.minimum(a,b)**spec['power']
...
def run(root,kind,mode):
    code=f'MIXTURE-CONCORDANCE-{kind.upper()}-{mode.upper()}'
    ...
    dev.json_write(trial/'contracts/RECIPE.json',{'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'mass_gate':'min(p_old_Mc_gap_tail,p_RNC_Mc_BC_tail)^power;each finite reference has1companionpair/source',
    ...
    for dep in e.DEPS:
        ...
        print(json.dumps({'mixture_concordance':code,'deployment':dep,'pools':[len(p) for p in pools],
