...
import mcwf_training_recipe_ensemble_20260907 as encoder
...
def calibration(root, dep, ms):
    encoder.prepare(root, dep, ms)
    folder = root / f'training_recipe_ensemble/models/{dep}/seed_{ms}'
...
def run(root, arm):
    trial = root / f'trials/TRAINING-RECIPE-ENSEMBLE-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'model': 'uniform predictive/kernel ensemble of random-view,mass-neighbor-batch,cross-image controls; each uses identical2s RAW-PHASE-SOURCE architecture and17epochs;three correlated parent models per paired seed,no new training',
