...
import mcwf_expanded_pair_features_20260907 as latent
...
def initialize(root):
    out=root/'expanded_pair_verifier';path=out/'contracts/TRAINING.json'
    if path.exists():return
    out.mkdir(parents=True,exist_ok=True)
    dev.json_write(path,{'created_utc':datetime.now(timezone.utc).isoformat(),
        'same_both_runs':True,'frozen_encoder':'frozen newly retrained joint-source density raw2s+phase encoder;352 phase/raw/merged hidden features',
        'network':'symmetric [abs(h_i-h_j),h_i*h_j]704->128SiLU dropout0.15->32SiLU->1logit',
        'training_sources':4096,'fit_views_per_source':8,'new_seeds':[202609251,202609252,202609253],
        'pairs':'each source4positive independent-view pairs/epoch;one negative per positive,half uniform,half sampled among32 nearest training waveform latents whose simulated logMc differs by at least0.15; positive imagesa/b independentnoise views',
        ...
        'selection':'minimum balanced-class BCE on all512-source encoder validation pairs,earliest tie',
        'calibration':'isotonic on selected logits with equal positive/negative totalweight; finite-source floor; cap+-4;positiveOOD0',
        'scoring':'Z_FRT+beta*verifier_increment;newpositiveincrementrequiresfrozenRNCmass-tail>=0.05;beta from BAYESTAR simulation validationonly with existing guardrails and priority',
...
def train(root,dep,ms,seed):
    initialize(root);out=root/f'expanded_pair_verifier/models/{dep}/seed_{ms}'
    if (out/'COMPLETE.json').exists():return
    out.mkdir(parents=True,exist_ok=True)
    tx=np.asarray(latent.hidden(root,dep,ms,0,'train'))
    vx=np.asarray(latent.hidden(root,dep,ms,0,'development'))
    trainpath=root/f'expanded_pair_verifier/features/{dep}/seed_{ms}/train.npy'
    ...
    imageA=[np.flatnonzero((groups==k)&(meta.image.to_numpy()=='a')) for k in range(len(names))]
    imageB=[np.flatnonzero((groups==k)&(meta.image.to_numpy()=='b')) for k in range(len(names))]
    masses=np.array([np.log(meta.mc_det.iloc[ids[0]]) for ids in members])
    centers=np.stack([tx[ids,-128:].mean(0) for ids in members])
    centers/=np.linalg.norm(centers,axis=1,keepdims=True).clip(1e-12)
    similarity=centers@centers.T
    incompatible=abs(masses[:,None]-masses[None,:])>=.15
    similarity[~incompatible]=-np.inf
    neighbors=np.argsort(-similarity,axis=1,kind='stable')[:,:32]
    if not np.isfinite(np.take_along_axis(similarity,neighbors,axis=1)).all():
        raise RuntimeError('Insufficient source-disjoint hard negatives')
    vi,vj=np.triu_indices(len(vx),1);vg=vm.source_uid.to_numpy();vy=(vg[vi]==vg[vj]);vweights=np.where(vy,.5/vy.sum(),.5/(~vy).sum())
    ...
    for epoch in range(first,EPOCHS+1):
        ...
        for _ in range(4):
            for g in rng.permutation(len(names)):
                u=int(rng.choice(imageA[g]));v=int(rng.choice(imageB[g]));a.append(u);b.append(v);labels.append(1.)
...
def raw_score(root,dep,ms,es,split):
    ...
    path=root/f'expanded_pair_verifier/predictions/{dep}/model_{ms}_eval_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
        ...
        return f,a['logits']
    hidden=np.asarray(latent.hidden(root,dep,ms,es,split))
    ck=torch.load(root/f'expanded_pair_verifier/models/{dep}/seed_{ms}/validation_selected_model.pt',weights_only=False,map_location='cpu')
...
def score(f,raw,cal):
    ...
    pt=tail.tail_probability(-np.log(f.new_mass_predictive_BC.to_numpy(float).clip(1e-12)),cal['mass_reference'])
    val=np.where(pt<.05,np.minimum(val,0.),val)
    return f.waveform_score.to_numpy(float)+cal['beta']*val,val,ood


def evaluate(root):
    trial=root/'trials/EXPANDED-SYMMETRIC-PAIR-VERIFIER'
    ...
    dev.json_write(trial/'contracts/RECIPE.json',{'training_contract_sha256':dev.sha(root/'expanded_pair_verifier/contracts/TRAINING.json'),
        'beta_grid':BETAS,'same_both_runs':True,'positive_screen':'no new reward if frozen RNC mass tail below0.05; original FRT unchanged','real_audit_is_adaptive_development':True,'selection':'existing validation priority and FRT-relative guardrails, real not opened to select beta'})
    selected={};statuses=[];cache={}
    for dep in e.DEPS:
        for ms,es in zip(body.MODEL_SEEDS,dev.SEEDS):
            f,raw=raw_score(root,dep,ms,es,'validation');cache[dep,es]=(f,raw)
            cal=json.loads((root/f'expanded_pair_verifier/models/{dep}/seed_{ms}/CALIBRATION.json').read_text());cal['mass_reference']=np.sort(-np.log(f.loc[f.is_true_pair,'new_mass_predictive_BC'].to_numpy(float).clip(1e-12))).tolist();bm=ev.metrics(f,f.waveform_score.to_numpy(float),dep,es)
            rows=[];choices=[]
            for beta in BETAS:
...
if __name__=='__main__':
    ...
    for dep in e.DEPS:
        for ms,seed in zip(body.MODEL_SEEDS,(202609251,202609252,202609253)):train(a.root,dep,ms,seed)
    evaluate(a.root)
