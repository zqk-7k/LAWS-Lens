...
OUT = Path("runs/current_data_qc_word_report_intuitive_20260622_logy_nofig")
DOC_ROOT = Path("docs")
...
def make_fig1(groups: dict[tuple[str, str], dict]) -> pd.DataFrame:
    fig, axes = plt.subplots(4, 2, figsize=(12.2, 10.5), constrained_layout=True)
    rows = []
    order = [("ET", "SIS"), ("ET", "PM"), ("LIGO", "SIS"), ("LIGO", "PM")]
...
def make_fig3(groups: dict[tuple[str, str], dict]) -> None:
    fig, axes = plt.subplots(4, 3, figsize=(13.4, 10.8), constrained_layout=True)
    order = [("ET", "PM"), ("ET", "SIS"), ("LIGO", "PM"), ("LIGO", "SIS")]
    for row, (detector, family) in enumerate(order):
...
def make_fig4(groups: dict[tuple[str, str], dict]) -> None:
    fig, axes = plt.subplots(4, 3, figsize=(13.4, 11.4), constrained_layout=True)
    order = [("ET", "PM"), ("ET", "SIS"), ("LIGO", "PM"), ("LIGO", "SIS")]
    for row, (detector, family) in enumerate(order):
        g = groups[(family, detector)]
        tr = g["trigger"]
...
def make_fig5(groups: dict[tuple[str, str], dict]) -> None:
    fig, axes = plt.subplots(4, 4, figsize=(13.4, 10.5), constrained_layout=True)
    order = [("ET", "PM"), ("ET", "SIS"), ("LIGO", "PM"), ("LIGO", "SIS")]
    for row, (detector, family) in enumerate(order):
        g = groups[(family, detector)]
        lp = g["lens_params"]
...
def make_fig6(groups: dict[tuple[str, str], dict]) -> None:
    ...
    fig_specs = [
        ("mass_distance", "GW source masses and distance", ["mass_1_source", "mass_2_source", "luminosity_distance"], (12.0, 6.2)),
        ("spin_orientation", "GW source spins and viewing angle", ["a_1", "a_2", "theta_jn"], (12.0, 6.2)),
        ("sky_phase", "GW source sky position and phase", ["ra", "dec", "psi", "phase"], (13.4, 6.2)),
        ("spin_angles_time", "GW source spin angles and event time", ["tilt_1", "tilt_2", "phi_12", "phi_jl", "geocent_time"], (14.2, 6.2)),
    ]
    for suffix, title, cols, size in fig_specs:
        fig, axes = plt.subplots(2, len(cols), figsize=size, constrained_layout=True)
        if len(cols) == 1:
...
def make_fig7_snr_block_hist(groups: dict[tuple[str, str], dict]) -> None:
    fig, axes = plt.subplots(3, 2, figsize=(10.8, 8.6), constrained_layout=True)
    for col, detector in enumerate(DETECTORS):
        blocks = []
...
def build_docx(groups: dict[tuple[str, str], dict]) -> None:
    sections = [
        ("代表性双像波形", OUT / "Fig1_lensed_pair_example_adaptive_peak.png", "用来看同一个源的 A 像（先到达）和 B 像（后到达）在峰值附近是否形态相似，峰值和幅度是否正常。本版对 ET/LIGO、SIS/PM 各展示一个代表样本。"),
        ("信号强度 SNR 分布", OUT / "Fig2_SNR_distribution_intuitive.png", "用来看 ET 和 LIGO 里信号强弱的整体分布，以及有多少样本超过 SNR=8 或 SNR=10。"),
        ...
        ("未透镜与不同透镜模型的 SNR 柱状分布", OUT / "Fig7_SNR_block_histogram_lens_models.png", "用类似论文示意图的方式展示 SNR 分布。A/B 两像不再分开，而是把同一透镜模型的两像合并；每个探测器分别展示 Unlensed、PM lensed 和 SIS lensed 三块。"),
    ]
    ...
    body = [
        ...
        p_xml("数据范围：当前 gw-catalog 代码 ROOTS 指向的 ET/LIGO、SIS/PM 最新数据。", None),
    ]
    rels, media = [], []
    ridn, docid = 1, 1
