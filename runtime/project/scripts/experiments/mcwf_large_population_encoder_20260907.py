...
def initialize(root):
    folder = root / 'large_population_encoder'
    ...
    dev.json_write(path, {
        ...
        'single_controlled_change': 'larger12288source training population and160independent noise blocks; same2s RAW-PHASE-SOURCE architecture,576templates,eventPSD,CE+SupCon+RNC loss',
        ...
        'step_budget': '12288/64*17=3264updates vs preceding4096/64*50=3200; update count differs by2percent, source/noise diversity is the main change; frozen before new-model fitting',
...
def train(root, dep, ms, seed):
    initialize(root)
    out = root / f'large_population_encoder/models/{dep}/seed_{ms}'
    ...
    for epoch in range(first, EPOCHS+1):
        ...
        if criterion < best:
            ...
            torch.save({**ck, 'model': {k: v.detach().cpu().clone() for k, v in model.state_dict().items()},
                ...
                'training_contract_sha256': dev.sha(root / 'large_population_encoder/contracts/TRAINING.json')}, out / 'validation_selected_model.pt')
...
def prediction(root, dep, ms, es, split):
    ...
    folder = root / f'large_population_encoder/predictions/{dep}/model_{ms}_eval_{es}'
