...
DESCRIPTIONS={
 ...
 'MCWF-END-TO-END-INTRINSICS-23':'在06轮低频与峰值2秒联合特征上，同时训练质量与条件自旋/质量比，共享表示接受完整联合分类损失。与同数据、同20epoch的纯质量头对照；checkpoint必须同时满足预先固定的质量交叉熵保护，未使用真实PE训练。',
 'MCWF-LARGE-MULTIRATE-24':'把06轮低频分支从4096源扩展到已有的全部12288训练源，包含160个独立噪声块。对额外8192源逐事件重新从物理信号和原噪声构造20-80Hz低频输入，原float16峰值2秒必须逐点一致。与相同12288源、同15epoch的纯原高频续训对照；两种模型均从原OMC初始化。',
}
