...
def combine(old, extra):
    return np.concatenate([old[:, 2].reshape(len(old), 64, 9), extra[:, 2].reshape(len(old), 64, 47)], 2)
...
def distribution(x, temperature):
    ...
    return lp, logsumexp(lp.reshape(len(x), 64, 56), axis=2)
...
def pair_features(x, i, j, temperature):
    ...
    for first in range(0, len(i), 512):
        ...
        full = np.log(3584)+logsumexp(lp[a]+lp[b], axis=1)
...
def fit(root, dep):
    path = root / f'dense_profile/{dep}/FIT.json'
    if path.exists():
        return json.loads(path.read_text())
    old = np.load(e.TRAINED / f'cache/finelag/{dep}/validation_features.npy')
    x = combine(old, dense.development(root, dep, 'validation'))
    ...
    print(json.dumps({'dense_profile_fit': dep, 'temperature': t, 'CE': min(r['Mc_CE'] for r in grid)}), flush=True)
    return specs


def get(root, dep, es, split, temperature):
    f = pd.read_parquet(e.BASE / f'evaluation/{dep}/seed_{es}/{split}_pairs.parquet')
    extra = dense.deployment(root, dep, es, split)
...
def run(root):
    trial = root / 'trials/DENSE-PHYSICAL-INTRINSIC-PROFILE'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'single_changed_mechanism': 'replace coarse64Mc*3q*3spin waveform profile with64Mc*8q*7spin grid; do not change2s input',
        'temperature_grid': coarse.TEMPERATURES, 'kinds': coarse.KINDS, 'beta_grid': coarse.BETAS,
        'calibration': 'same simulation source-balanced validation isotonic,cap+-4,positiveOOD0',
        'selection': 'BAYESTAR validation only, same objective and FRT-relative guardrails',
        'limits': 'maximized phase/time profiles are NOT physical PE posteriors,properBF,or a template-bank minimal-match guarantee; Mc temperature coverage does not establish q/spin calibration',
    ...
    print(json.dumps({'dense_profile_selected': states}), flush=True)
    rows, guards = [], []
    for dep in e.DEPS:
        real = {}
        for es in dev.SEEDS:
            ...
            for split in ('validation', 'test', 'real'):
                ...
                n['waveform_score'], n['dense_profile_increment'], n['dense_profile_ood'] = z, inc, ood
                for k, kind in enumerate(coarse.KINDS):
                    n['dense_profile_'+kind] = x[:, k]
