...
def run_family(family: str, out_root: Path):
    ...
    result = {'family': family, 'mode': MODE, 'method': 'catalog_level_delta_time_sky_sep_waveform_hgb', 'features': ['log1p_delta_time', 'sky_sep', 'waveform_score', 'waveform_reciprocal_rank'], 'neg_per_pos': NEG_PER_POS, 'train_examples': int(len(yv)), 'train_positive': int(yv.sum()), 'val_auc_sampled': float(roc_auc_score(yv, pv)), **eval_full(clf, test_obs, test_gt, test_scores, test_ranks)}
    print(result, flush=True)
    return result


def main():
    out_root = Path('runs/ligo_noisy_catalog_fusion_aux_waveform_20260601')
