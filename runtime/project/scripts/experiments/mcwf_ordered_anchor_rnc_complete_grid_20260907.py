...
import mcwf_ordered_anchor_rnc_evaluate_20260907 as anchor
...
def run(root, model, arm):
    anchor.MODE = model
    module = SimpleNamespace(GAMMAS=anchor.GAMMAS,BETAS=anchor.BETAS,values=anchor.values,calibrate=anchor.calibration,score=lambda f,x,spec,arm: anchor.score(f,x,spec)[:3])
    code = f'ORDERED-ANCHOR-RNC-COMPLETE-GRID-{model.upper()}'
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'formula': 'Z_FRT+gamma*orderedMcCNNtail+beta*frozen source-kernelLR;new positive reward requires orderedCNN mass compatibility and no mass boundary OOD',
...
if __name__ == '__main__':
    ...
    for model in ('large', 'ensemble'):
        run(args.root, model, 'MASS-COSINE')
