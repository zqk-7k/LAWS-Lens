...
import mcwf_mass_transport_evaluate_20260907 as transport
...
def run(root, model, arm):
    ...
    module = SimpleNamespace(GAMMAS=transport.GAMMAS, BETAS=transport.BETAS,
        values=transport.values, score=transport.score,
        calibrate=lambda r,d,m: transport.calibrate(r,d,m,dev.SEEDS[body.MODEL_SEEDS.index(m)])[arm])
    code = f'MASS-TRANSPORT-COMPLETE-GRID-{model.upper()}-{arm}'
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'formula': 'Z_FRT+gamma*finite_companion_distance_tail+beta*bounded_isotonic_negative_distance_LR;distance=W1(logMc predictive distribution) or absolute mean gap',
...
if __name__ == '__main__':
    ...
    for model in ('frozen', 'large', 'ensemble'):
        for arm in ('W1', 'MEAN'):
