...
def main():
    ...
    for dep in n.DEPS:
        ...
        for kind in ('NN','R10PROFILE','BOUNDED'):
            center=(r.mass_summary(a['parent_p' if kind=='NN' else 'p'])[:,0]
                    if kind!='R10PROFILE' else a['profile_center'])
            active=(np.ones(len(center),bool)if kind=='NN' else
