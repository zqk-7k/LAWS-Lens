...
import mcwf_fine_mass_density_20260907 as mdn
...
def fit(root,dep,ms):
    path=root/f'fine_mass_density/calibration/{dep}/seed_{ms}/fit.json'
    if path.exists():return json.loads(path.read_text())
    a=np.load(root/f'fine_mass_density/models/{dep}/seed_{ms}/validation_predictions.npz')
    ...
    spec={'positive_source_pairs':int(y.sum()),'source_count':len(np.unique(a['group'])),
        ...
        'checkpoint_sha256':dev.sha(root/f'fine_mass_density/models/{dep}/seed_{ms}/validation_selected_model.pt')}
...
def get(root,dep,ms,es,split):
    ...
    path=root/f'fine_mass_density/pair_features/{dep}/seed_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
...
def run(root):
    trial=root/'trials/FINE-MC-INTRINSIC-DENSITY'
    ...
    dev.json_write(trial/'contracts/RECIPE.json',{'created_utc':datetime.now(timezone.utc).isoformat(),
        ...
        'predictive_targets':'logMc,logitq,atanhchieff;4componentfullcovarianceMDN;waveform-only2sphysicaltemplateswith253massgridrefinement',
    ...
    print(json.dumps({'fine_mc_selected':states}),flush=True)
    rows,guards=[],[]
    for dep in e.DEPS:
        real={}
        for ms,es in zip(body.MODEL_SEEDS,dev.SEEDS):
