...
OUT_ROOT = Path('runs/ligo_sis_detector_interaction_expected_angular_20260605')
...
class SkyMapDataset(Dataset):
    def __init__(self, ds: EvaluationSet, raw_obs: pd.DataFrame) -> None:
        ...
        self.unit = base.unit_vectors(raw_obs).astype(np.float32)

    def __len__(self) -> int:
        return len(self.ds)

    def __getitem__(self, idx: int):
        return self.ds[idx], torch.from_numpy(self.y[idx]), torch.from_numpy(self.unit[idx])

def detector_interaction_channels(x: torch.Tensor) -> torch.Tensor:
    if x.shape[1] != 2:
        return x
    h = x[:, 0:1]
    l = x[:, 1:2]
    return torch.cat([h, l, h - l, torch.abs(h - l), h * l], dim=1)
...
class SkyMapCNN(nn.Module):
    def __init__(self, in_channels: int = 2, n_pix: int = N_PIX) -> None:
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(5 if in_channels == 2 else in_channels, 48, 31, stride=4, padding=15, bias=False),
            nn.BatchNorm1d(48),
            nn.GELU(),
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.net(self.stem(detector_interaction_channels(x.float())))
...
def train_model(job, train_ds, train_raw, val_ds, val_raw, out_dir):
    ...
    center_unit_t = torch.from_numpy(CENTER_UNIT).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        model.train(); losses = []
        for x, y, unit in train_loader:
            x = x.to(device); y = y.to(device); unit = unit.to(device)
            logits = model(x)
            logp = F.log_softmax(logits, dim=1)
            prob = torch.softmax(logits, dim=1)
            loss_kl = F.kl_div(logp, y, reduction='batchmean')
            expected_vec = prob @ center_unit_t
            expected_vec = F.normalize(expected_vec, dim=1)
            loss_ang = torch.mean(1.0 - torch.sum(expected_vec * unit, dim=1))
            loss = loss_kl + ANGULAR_LAMBDA * loss_ang
...
def run_job(job):
    ...
    row = {
        ...
        'method': 'ligo_sis_detector_interaction_expected_angular_rerank',
        'features': 'log1p_delta_time_obs,detector_interaction_expected_angular_skymap_overlap,waveform_score,waveform_reciprocal_rank',
        'sky_model': f'SkyMapCNN_detector_interaction_expected_angular_grid_{DEC_BINS}x{RA_BINS}',
