...
OUT_ROOT = Path("runs/time_matched_full_catalog_ranking_20260611")
...
def family_by_index(meta: list[dict], rows: np.ndarray) -> np.ndarray:
    return np.asarray([meta[int(i)]["family"] for i in rows])
...
def full_catalog_composition(meta: list[dict]) -> dict:
    ...
    for family in FAMILIES:
        rows.append({
            "family": family,
            "lensed_images": sum(1 for item in meta if item["family"] == family and item["tag"] in {"L1", "L2"}),
            "unlensed": sum(1 for item in meta if item["family"] == family and item["tag"] == "U"),
        })
    out = {"catalog_total": int(len(meta))}
    for row in rows:
        prefix = row["family"].lower()
        out[f"{prefix}_lensed_images"] = int(row["lensed_images"])
        out[f"{prefix}_unlensed"] = int(row["unlensed"])
    out["total_lensed_images"] = int(sum(row["lensed_images"] for row in rows))
    out["total_unlensed"] = int(sum(row["unlensed"] for row in rows))
    return out
...
def add_group_rows(
    ...
    for subset, values in metrics.items():
        rows.append({
            "detector": detector,
            "data_mode": mode,
            "catalog": "mixed_SIS_PM_full_catalog",
...
def run_one(detector: str, mode: str) -> list[dict]:
    ...
    for variant in DIRECT_VARIANTS:
        print("DIRECT_FULL", detector, mode, variant, flush=True)
        ...
        pd.DataFrame(all_rows).to_csv(out_dir / "full_catalog_partial.csv", index=False)

    for index, variant in enumerate(RERANK_VARIANTS):
        print("RERANK_FULL", detector, mode, variant, flush=True)
        ...
        pd.DataFrame(all_rows).to_csv(out_dir / "full_catalog_partial.csv", index=False)
    pd.DataFrame(all_rows).to_csv(out_dir / "full_catalog_summary.csv", index=False)
    return all_rows
...
def main() -> None:
    ...
    for detector, mode in JOBS:
        print("RUN_FULL_CATALOG", detector, mode, flush=True)
        all_rows.extend(run_one(detector, mode))
        pd.DataFrame(all_rows).to_csv(OUT_ROOT / "full_catalog_summary_partial.csv", index=False)
    df = pd.DataFrame(all_rows)
    df.to_csv(OUT_ROOT / "full_catalog_summary.csv", index=False)
    ...
    summary = {
        ...
        "candidate_kind": "full_catalog",
        "note": "Validation rerankers are trained with time-matched hard negatives; test ranks against the full mixed catalog including unlensed events.",
    }
