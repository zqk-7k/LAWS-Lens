#!/usr/bin/env python3
"""Re-score GWTC-3/O3 and GWTC-4.1/O4a with unified sky-v8.
...
posterior statistics, validation-frozen sky calibration, fusion weights, and
downstream rankings are recomputed.
"""
...
def _rank_real(
    ...
    real = attach_sky_features(base, sky, calibration)
    event_diagnostics["event_name"] = primary["event_name"].astype(str).to_numpy()
    event_diagnostics.to_parquet(
        output_seed / "real_event_sky_diagnostics_v8.parquet",
        index=False,
    )
    real.to_parquet(output_seed / "real_pair_features_unified_sky_v8.parquet", index=False)
    ...
    for method, weights in methods.items():
        ...
        all_catalog.to_parquet(
            output_seed / f"real_pair_scores_all_catalog_{method}_v8.parquet",
            index=False,
        )
        strict.to_parquet(
            output_seed / f"real_pair_scores_strict_h1l1_bbh_{method}_v8.parquet",
            index=False,
        )
        all_catalog.head(100).to_csv(
            output_seed / f"candidate_shortlist_all_catalog_{method}_v8.csv",
            index=False,
        )
        strict.head(100).to_csv(
            output_seed / f"candidate_shortlist_strict_h1l1_bbh_{method}_v8.csv",
            index=False,
        )
...
def run_seed(
    ...
    calibration = fit_sky_calibration(
        val_sky,
        validation_base["is_true_pair"].to_numpy(dtype=np.int8),
        deployment=deployment,
        seed=seed,
    )
    write_json(output_seed / "sky_calibration_v8.json", calibration)
    val_event_sky.to_parquet(
        output_seed / "synthetic_validation_event_sky_diagnostics_v8.parquet",
        index=False,
    )
    test_event_sky.to_parquet(
        output_seed / "synthetic_test_event_sky_diagnostics_v8.parquet",
        index=False,
    )

    validation = attach_sky_features(validation_base, val_sky, calibration)
    heldout = attach_sky_features(test_base, test_sky, calibration)
    validation.to_parquet(
        output_seed / "fusion_validation_pairs_v8.parquet",
        index=False,
    )
    heldout.to_parquet(
        output_seed / "fusion_heldout_test_pairs_v8.parquet",
        index=False,
    )
    ...
    for name, grid in grids.items():
        grid.to_csv(output_seed / f"fusion_weight_grid_{name}_v8.csv", index=False)
    ...
    validation_pair.to_csv(
        output_seed / "validation_pair_level_metrics_v8.csv",
        index=False,
    )
    test_query.to_parquet(output_seed / "heldout_test_query_ranks_v8.parquet", index=False)
    test_metrics.to_csv(output_seed / "heldout_test_retrieval_metrics_v8.csv", index=False)
    test_pair.to_csv(output_seed / "heldout_test_pair_level_metrics_v8.csv", index=False)
    ...
    bootstrap.to_csv(
        output_seed / "heldout_test_bootstrap_95ci_v8.csv",
        index=False,
    )
    ...
    write_json(output_seed / "waveform_gate_validation_v8.json", gate)
    ...
    real_map_audit.to_csv(output_seed / "real_map_source_audit_v8.csv", index=False)
    ...
    primary_all.head(100).to_csv(
        output_seed / "candidate_shortlist_all_primary_v8.csv",
        index=False,
    )
    primary_strict.head(100).to_csv(
        output_seed / "candidate_shortlist_strict_primary_v8.csv",
        index=False,
    )
    summary = {
        ...
        "sky_validation": calibration["validation"],
...
def aggregate(deployment: str, seeds: tuple[int, ...], output_root: Path) -> None:
    ...
    retrieval = pd.concat(
        [
            pd.read_csv(root / f"seed_{seed}/heldout_test_retrieval_metrics_v8.csv")
            for seed in seeds
        ],
        ignore_index=True,
    )
    pairs = pd.concat(
        [
            pd.read_csv(root / f"seed_{seed}/heldout_test_pair_level_metrics_v8.csv")
            for seed in seeds
        ],
        ignore_index=True,
    )
    bootstrap = pd.concat(
        [
            pd.read_csv(root / f"seed_{seed}/heldout_test_bootstrap_95ci_v8.csv")
            for seed in seeds
        ],
        ignore_index=True,
    )
    retrieval.to_csv(root / "heldout_test_retrieval_metrics_per_seed_v8.csv", index=False)
    pairs.to_csv(root / "heldout_test_pair_level_metrics_per_seed_v8.csv", index=False)
    bootstrap.to_csv(
        root / "heldout_test_bootstrap_95ci_per_seed_v8.csv",
        index=False,
    )
    _numeric_summary(retrieval, ["deployment", "method", "subset"]).to_csv(
        root / "heldout_test_retrieval_metrics_across_seed_v8.csv",
        index=False,
    )
    _numeric_summary(pairs, ["deployment", "method"]).to_csv(
        root / "heldout_test_pair_level_metrics_across_seed_v8.csv",
        index=False,
    )
    ...
    for seed in seeds:
        ...
        gate = json.loads((seed_root / "waveform_gate_validation_v8.json").read_text())
        gate_rows.append(
            {
                "deployment": deployment,
                "seed": seed,
        ...
        summary = json.loads((seed_root / "seed_summary_v8.json").read_text())
        method = summary["primary_method"]
        scored = pd.read_parquet(
            seed_root / f"real_pair_scores_strict_h1l1_bbh_{method}_v8.parquet"
        )
        real_rows.append(
            scored[
                [
    ...
    weights.to_csv(root / "selected_weights_per_seed_v8.csv", index=False)
    pd.DataFrame(gate_rows).to_csv(root / "waveform_gate_per_seed_v8.csv", index=False)

    rank_stability = pd.concat(real_rows, ignore_index=True)
    rank_stability.to_parquet(root / "real_candidate_rank_stability_v8.parquet", index=False)
    consensus = (
    ...
    consensus.to_parquet(root / "real_candidate_consensus_v8.parquet", index=False)
    consensus.head(100).to_csv(root / "real_candidate_consensus_top100_v8.csv", index=False)
