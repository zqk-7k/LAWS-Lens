...
def comparison(root):
    ...
    if 'SCALE_INPUT_INVALIDITY' in ' '.join(p.name for p in (root/'contracts').iterdir()):
        raise RuntimeError('Invalid SCALE experiment must not receive an ordinary performance report')
    ...
    lines=[f'# {ident} 独立机制对照报告','',
      '本轮计算完成不代表共同改善目标已经完成。历史保留版 OMC、时间、天空、外层权重和候选范围没有覆盖。',
    ...
    lines+=['','共同通过且仍需独立确认的开发配置：'+(', '.join(winners) if winners else '无。本轮不能替换保留版。'),'',
      ...
      '时间和天空逐pair最大差严格为0，保护文件SHA-256复查见manifest/PROTECTED_RECHECK.csv。','',t.STATUS]
    (root/'reports/ROUND_REPORT_CN.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
    font=configure_plot()
    methods=sorted(seeds.method.unique(),key=lambda s:(s!='OMC',s))
    fig,axes=plt.subplots(2,3,figsize=(max(9,len(methods)*1.9),7),layout='constrained')
    for i,dep in enumerate(t.DEPS):
        ...
        for j,(column,label) in enumerate([('macro_r_at_10','Companion R@10'),('average_precision','Pair AUPRC'),('false_at_recall_0p9','False pairs at 90% recall')]):
            ...
            for k,name in enumerate(methods):
                ...
                ax.scatter(k+np.linspace(-.12,.12,len(values)),values,s=22,color=plt.get_cmap('tab10')(k),alpha=.85)
                if len(values):ax.plot([k-.2,k+.2],[np.median(values)]*2,color='black',lw=1)
            ax.set_xticks(range(len(methods)),[x.replace('-','\n',1) for x in methods],rotation=25,ha='right')
            ax.set_ylabel(label);ax.set_title(dep.replace('gwtc3','O3').replace('gwtc4','O4a'));ax.grid(axis='y',alpha=.18)
    ...
    for i,dep in enumerate(t.DEPS):
        ...
        for j,(column,label) in enumerate([('BC_mc_ge_0p5','Mc BC >= 0.5'),('official_frontend','Official 1% frontend overlap'),('official_hanabi','Published Hanabi-table overlap')]):
            ...
            for b,offset,color in [(10,-.17,'#14857B'),(20,.17,'#B75975')]:
                ...
                ax.bar(np.arange(len(methods))+offset,vals,width=.3,color=color,label=f'Top-{b}')
            ax.set_xticks(range(len(methods)),[x.replace('-','\n',1) for x in methods],rotation=25,ha='right')
            ax.set_ylabel(label);ax.set_title(dep.replace('gwtc3','O3').replace('gwtc4','O4a'));ax.set_ylim(0,21)
    axes[0,0].legend(frameon=False,loc='upper left');savefig(fig,root,'fig_real_PE_official_top10_top20')
...
if __name__=='__main__':
    ...
    p.add_argument('--stage',choices=['comparison','pilot','seal'],required=True);a=p.parse_args()
