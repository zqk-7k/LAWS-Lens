...
def freeze(root, data, expansion, reference, catalog):
    ...
    contract = {'UTC': utc(), 'id': 'MCWF-GLOBAL-WAVEFORM-BRANCH-CALIBRATION-64',
        ...
        'implementation_recovery': 'Original R64 stopped before fitting due initializer arity; R64B stopped before fitting because an inventory assertion wrongly expected profiles for high-mass events outside frozen scope. R64C also stopped before fitting: plans include both images when either predicts Mc<=15, not only the low-predicted-mass image. R64D reconstructs that exact frozen source-level plan, validates every profile/hash, and retains the original eventwise eligibility. No scoring or scientific rule changes.',
    ...
    for dep in DEPS:
        ...
        files += [data / f'contracts/{dep}_PROFILE_EVENT_PLAN.parquet',
                  data / f'contracts/{dep}_PROFILES_COMPLETE.json']
...
def population(data, dep, pairs, inventory):
    ...
    summary = predictive.mode.r.mass_summary(parent)
    low_mass = np.isfinite(summary[:, 0]) & (np.exp(summary[:, 0]) <= 15.)
    expected_sources = set(meta.source_uid[low_mass])
    expected = set(meta.row_index[meta.source_uid.isin(expected_sources)])
    actual = [int(r['row_index']) for r in records]
    plan = pd.read_parquet(data / f'contracts/{dep}_PROFILE_EVENT_PLAN.parquet')
    receipt = json.loads((data / f'contracts/{dep}_PROFILES_COMPLETE.json').read_text())
    if (len(actual) != len(set(actual)) or set(actual) != expected or
            set(plan.row_index) != expected or receipt['events'] != len(actual) or
            receipt['plan_sha256'] != sha(data / f'contracts/{dep}_PROFILE_EVENT_PLAN.parquet')):
        raise RuntimeError('Missing or unexpected frozen low-mass profile, or plan hash mismatch')
    ...
    for fold in (0, 1):
        ...
        audits.append({'deployment': dep, 'fold': fold, 'events': len(ids), 'sources': size,
            ...
            'frozen_profile_scope_events': int(sum(i in expected for i in ids)),
            'planned_profile_missing': 0, 'unprofiled_scope_is_ineligible_not_missing': True,
