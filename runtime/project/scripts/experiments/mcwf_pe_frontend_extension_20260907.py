...
def fit_reference(dep, ms, recipe, prior):
    ...
    p = softmax(a['logits'].astype(float), axis=1)
    g = a['group']
    i, j = np.triu_indices(len(p), 1)
    y = g[i] == g[j]
    x = features(p, a['embedding'], i, j, prior)[recipe.split('-')[0]]
    spec = {'recipe': recipe, 'prior': prior.tolist(), 'fit_positive_systems': int(y.sum()),
...
def run_trial(root, recipe):
    trial = root / 'trials' / recipe
    if trial.exists():
...
if __name__ == '__main__':
    ...
    else: run_trial(args.root, args.recipe)
