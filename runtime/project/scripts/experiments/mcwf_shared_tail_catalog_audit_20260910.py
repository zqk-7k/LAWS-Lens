#!/usr/bin/env python3
"""R65 post-freeze tail audit; thresholds and guards are inherited."""
import os
...
def main(root):
    ...
    reference, pilot = Path(contract['reference']), Path(contract['population'])
    ...
    for path in files:
        ...
        for method in app.METHODS:
            ...
            if method == app.NEW:
                flagged = active & (frame.shared_profile_deficit.to_numpy() > cfg['tail_calibration']['cutoff_D'])
                expected = nd.waveform_score.to_numpy().copy()
                expected[flagged] = np.minimum(expected[flagged], previous.waveform_score.to_numpy()[flagged])
                if not np.array_equal(expected, frame.waveform_score.to_numpy()):
                    raise RuntimeError('Frozen selective lower-envelope identity failed')
                if np.any(frame.waveform_score.to_numpy() > nd.waveform_score.to_numpy()):
                    raise RuntimeError('Tail-only rule increased waveform')
                ...
                for pos in np.flatnonzero(changed):
                    changes.append({**frame.iloc[pos][ids].to_dict(), 'deployment': dep,
                        'seed': seed, 'panel': panel, 'R62_waveform': float(previous.waveform_score.iloc[pos]),
            ...
            checks.append({'method': method, 'deployment': dep, 'seed': seed, 'panel': panel,
                ...
                'changed_from_R62': int(changed.sum()), 'frozen_channels_exact': True,
    ...
    shutil.copy2(__file__, root / 'scripts/shared_tail_catalog_audit.py')
