...
SEEDS=(202609241,202609242,202609243)
...
def development(root,dep,split):
    ...
    return full_density.inputs(root,dep,split),meta
...
def initialize(root):
    out=root/'fine_mass_only';path=out/'contracts/TRAINING.json'
    ...
    dev.json_write(path,{'created_utc':datetime.now(timezone.utc).isoformat(),'same_O3_O4a':True,
        ...
        'comparison':'samefineMcfeatures,60epochs,3percentstandardizedscale_floor;removeq/spinjointNLL tofocusonMc',
...
def train(root,dep,ms,seed):
    initialize(root);out=root/f'fine_mass_only/models/{dep}/seed_{ms}'
...
def prediction(root,dep,ms,es,split):
    cp=root/f'fine_mass_only/models/{dep}/seed_{ms}/validation_selected_model.pt'
    if not (cp.parent/'COMPLETE.json').exists():raise RuntimeError('Trainingincomplete')
    path=root/f'fine_mass_only/predictions/{dep}/model_{ms}_eval_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
