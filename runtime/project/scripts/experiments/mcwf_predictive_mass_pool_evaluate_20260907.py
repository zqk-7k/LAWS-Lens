...
import mcwf_predictive_mass_pool_20260907 as mass_model
...
def run(root, arm):
    trial = root / f'trials/HETEROGENEOUS-MASS-{mass_model.MODE.upper()}-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'arm': arm, 'pooling': mass_model.MODE, 'model': 'development-CE-calibrated pool of frozen raw RNC, fine-Mc-only MDN and higher-mode MDN marginal; same2sinput',
