...
NEG_PER_POS = 500
CHUNK_ROWS = 64
FEATURES = ['log1p_delta_time', 'sky_sep', 'chirp_mass_logdiff', 'mass_ratio_absdiff', 'chi_eff_absdiff', 'luminosity_distance_logdiff']
...
def feature_matrix(obs: pd.DataFrame, anchors: np.ndarray, cands: np.ndarray) -> np.ndarray:
    ...
    cm = obs['chirp_mass'].to_numpy(); q = obs['mass_ratio'].to_numpy(); chi = obs['chi_eff_proxy'].to_numpy(); dl = obs['luminosity_distance'].to_numpy()
...
def run_family(family: str, out_root: Path):
    ...
    result = {'family': family, 'mode': MODE, 'method': 'catalog_level_extended_observable_hgb', 'features': FEATURES, 'neg_per_pos': NEG_PER_POS, 'train_examples': int(len(yv)), 'train_positive': int(yv.sum()), 'val_auc_sampled': float(roc_auc_score(yv, pv)), **eval_full(clf, test_obs, test_gt)}
    print(result, flush=True)
    return result


def main():
    out_root = Path('runs/ligo_noisy_catalog_extended_observable_20260601')
