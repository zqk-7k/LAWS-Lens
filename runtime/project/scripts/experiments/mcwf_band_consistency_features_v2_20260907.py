...
def geometry(bank):
    ...
    energy = trace[..., 1:] - trace[..., :-1]
    peak_fraction = energy.amax(-1) / trace[..., -1]
    counts = torch.floor(1 / peak_fraction).long().clamp(2, BINS)
    fraction = (torch.arange(1, BINS, device='cuda') / counts[..., None]).clamp_max(1.)
    boundaries = torch.searchsorted(trace.contiguous(), (trace[..., -1, None] * fraction).contiguous())
    boundaries = torch.where(torch.arange(1, BINS, device='cuda') >= counts[..., None], len(weight), boundaries)
    ...
    if not torch.equal(nonempty.sum(-1), counts):
        raise RuntimeError('Adaptive frequency partition count mismatch')
    ...
    safe_blocks = torch.where(nonempty[..., None, None], blocks, torch.eye(2, device='cuda'))
    eig = torch.linalg.eigvalsh(safe_blocks)
    if eig.min() < 1e-7:
        raise RuntimeError('Ill-conditioned active subband Gram matrix')
    inverse = torch.linalg.inv(safe_blocks) * nonempty[..., None, None]
    ...
    return kernels, weight, edges, blocks, inverse, identity_error, (2 * counts - 2)


@torch.no_grad()
def extract(values, bank, batch=12, normalize=True, maximize_lag=True):
    kernels, weight, edges, blocks, inverse, identity_error, dof = geometry(bank)
    ...
    return np.concatenate(powers), np.concatenate(residuals), {
        ...
        'minimum_effective_bin_count': int((dof.min().item() + 2) / 2),
        'maximum_effective_bin_count': int((dof.max().item() + 2) / 2)}, dof.T.cpu().numpy()
...
def tests(bank):
    ...
    p, r, audit, dof = extract(pure, small, normalize=False, maximize_lag=False)
    ...
    _, statistic, _, dof = extract(noise, small, normalize=False, maximize_lag=False)
...
def initialize(root):
    ...
    dev.json_write(contract, {'created_utc': datetime.now(timezone.utc).isoformat(),
        ...
        'maximum_bins': BINS, 'partitions': 'contiguous equal-power bands; pertemplate/detector count=min(8,max(2,floor(1/largest-frequency-cell-energy-fraction))); empty padded bands have zero contribution',
        'implementation_revision': 'v1 rejected a training PSD with narrow high-Mc templates; no v1 model fitted. Adaptive count corrects empty-bin algebra, not an outcome threshold; prior failure log retained',
        'formula': 'a=Q*x at best full-power lag; G_b=Q F_b Q^T; r_b=Q F_b x-G_b a; residual=sum r_b^T G_b^-1 r_b',
        'features': 'original1728 powers plus1152 log1p(residual/actual-dof) and1152 actual-dof/14 features;4032 total; no new frequency band, window, time or sky information',
...
def grouped(root, raw, freq, psds, indices, out, original=None):
    ...
    result = np.empty((len(raw), 4, 576), np.float32)
    rows = []
    for pid in np.unique(indices):
        take = np.flatnonzero(indices == pid)
        bank = adaptive.whitened_bank(h, freq, psds[int(pid)])
        test_path = root / 'band_consistency_v2/contracts/NUMERICAL_TESTS.json'
        if not test_path.exists():
            dev.json_write(test_path, tests(bank))
        started = time.perf_counter()
        power, residual, audit, dof = extract(raw[take], bank)
        ...
        result[take, :2] = np.log1p(residual / dof)
        result[take, 2:] = dof[None] / (2 * BINS - 2)
...
def development(root, dep, split):
    ...
    new = grouped(root, raw, np.load(folder / 'noise/frequency.npy'), np.load(folder / 'noise/psd.npy'),
                  meta.noise_bank_index.to_numpy(int), root / f'band_consistency_v2/features/{dep}/{split}.npy', original)
...
def deployment(root, dep, es, split):
    ...
    out = root / f'band_consistency_v2/features/{dep}' / ('real.npy' if split == 'real' else f'{es}_{split}.npy')
