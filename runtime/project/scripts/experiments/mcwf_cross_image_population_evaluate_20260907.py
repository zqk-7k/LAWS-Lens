...
import mcwf_cross_image_population_encoder_20260907 as encoder
...
def calibration(root, dep, ms):
    folder = root / f'cross_image_population_encoder/models/{dep}/seed_{ms}'
...
def run(root, arm):
    trial = root / f'trials/CROSS-IMAGE-POPULATION-RNC-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'model': 'unchanged 2s RAW-PHASE-SOURCE RNC,576 templates;17epochs,3264updates vs previous3200',
