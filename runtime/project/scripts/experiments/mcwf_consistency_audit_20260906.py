...
def run(root):
    ...
    mismatches=[str(r.path) for r in protected.itertuples() if dev.sha(Path(r.path))!=r.sha256]
    dev.json_write(root/'audit/PROTECTED_INPUT_HASH_RECHECK.json',{
        'checked_files':len(protected),'mismatches':mismatches,'pass':not mismatches})
    if mismatches:
        raise RuntimeError('Protected historical input hash mismatch')
    ...
    for dep in u.DEPS:
        for ms,es in zip(body.MODEL_SEEDS,dev.SEEDS):
            ...
            rows[-1]['rankncontrast']=bool(ck.get('rankncontrast',False))
    f=pd.DataFrame(rows)
    ...
    implementation=(finite_tail.score if f.recipe.iloc[0]=='finite_reference_companion_tail'
                    else conditional.score if f.recipe.iloc[0]=='conditional_monotone_waveform'
    ...
    dev.json_write(root/'contracts/METHOD_CONSISTENCY_AUDIT.json',{'pass':True,'all_six_same_parameter_shapes':True,'all_six_same_score_formula':True,
        ...
        'parameters_may_differ':'deployment-specific trained weights, validation normalization/calibration and numeric hyperparameters; never formula or input channels',
        'training_noise_caveat':'legacy O3 development training includes O1/O2 noise; it is not fully O3-run-matched. Fresh confirmation uses O3-only noise.'})
