#!/usr/bin/env python3
"""Shared single-detector predictive mass model and H1/L1 consistency audit."""
import os
...
class Predictor(nn.Module):
    def __init__(self):
        super().__init__()
        self.net=nn.Sequential(nn.Linear(3584,256),nn.LayerNorm(256),nn.SiLU(),nn.Dropout(.1),
            nn.Linear(256,128),nn.SiLU(),nn.Linear(128,12))

    def forward(self,x):
        a=self.net(x).reshape(-1,4,3)
        return a[...,0],a[...,1],F.softplus(a[...,2])+.015
...
def matrix(coarse,extra):
    return np.concatenate([np.concatenate([coarse[:,d].reshape(len(coarse),-1),extra[:,d]],1) for d in (0,1)],0)


def development(root,dep,split):
    meta=pd.read_parquet(root/f'expanded_data/{dep}/{split}/event_metadata.parquet')
    a=np.load(root/f'expanded_encoder/features/{dep}/{split}.npy')
    b=np.load(root/f'mixture_density/features/{dep}/{split}_extra.npy')
    return matrix(a,b),meta
...
def pooled(p,prior,eta):
    n=len(p)//2
    logp=(1-2*eta)*np.log(prior.clip(1e-300))+eta*(np.log(p[:n])+np.log(p[n:]))
    return softmax(logp,1)


def initialize(root):
    out=root/'detector_mass_density';path=out/'contracts/TRAINING.json'
    if path.exists():return
    dev.json_write(path,{'created_utc':datetime.now(timezone.utc).isoformat(),'same_O3_O4a':True,
        'input':'peak2s4096points;single-detector event-PSD phasequadrature3584templatefeatures;one sharednetworkforH1andL1,no detectorID',
        'population':'expanded4096training/512validationsources;96/32disjointnoiseblocks;allhistoricaldatasetsunchanged',
        'architecture':'3584->256LN/SiLU/dropout0.1->128SiLU->4Gaussian(logMc)mixturecomponents',
        'targets':'continuousdetector-framelogMc','training':'80epochs,128sourceparents*2views*2detectors,onepass/epoch,AdamW2e-4,WD1e-4,cosine_min1e-5,clip5',
        'selection':'earliestminimumsingle-detectorvalidationNLL;variance-temperaturebyvalidationNLL;networkpoolingeta byvalidation predictivecrossentropy',
        'variance_temperatures':[.5,.75,1,1.25,1.5,2],'pooling_eta':[.25,.5,.75,1.],
        'pooling_formula':'p_HL proportional prior^(1-2eta)*p_H^eta*p_L^eta',
        'prior':'one histogram entry pertraining source;logMc5-200,512bins,Gaussian smoothing2bins,pseudocount1e-3/bin',
        'statistical_limit':'temperedpredictiveaggregation,NOT a rigorousnetworkPEposterior:sharednuisanceparametersare notjointlymarginalized by multiplying1Dmarginals',
        'detector_check':'relativepredictiveMccompatibility only;notrealposterioroverlap orlensingBF',
        'seeds':SEEDS,'scale_floor':.015,'frozen':['time','sky','outerweights','scope','historicaloutputs'],
        'no_real_PE_official_or_ID_training':True,'fresh_confirmation_required':True})
    (out/'scripts').mkdir(exist_ok=True);shutil.copy2(__file__,out/'scripts'/Path(__file__).name)


def train(root,dep,ms,seed):
    initialize(root);out=root/f'detector_mass_density/models/{dep}/seed_{ms}'
    ...
    yy=torch.as_tensor(np.tile((truth-ym)/ys,2),dtype=torch.float32,device='cuda');vy=torch.as_tensor(np.tile((vt-ym)/ys,2),dtype=torch.float32,device='cuda')
    ...
    for epoch in range(first,EPOCHS+1):
        ...
        for begin in range(0,len(order),128):
            ...
            indices=np.r_[take,take+len(tm)];opt.zero_grad(set_to_none=True);loss=nll(model(xx[indices]),yy[indices])
            if not torch.isfinite(loss):raise RuntimeError('NonfinitesingledetectorNLL')
        ...
        if epoch%20==0:print(json.dumps({'detector_mass_training':dep,'seed':seed,**history[-1]}),flush=True)
    ...
    targetbin=np.clip(np.searchsorted(EDGES,vt,side='right')-1,0,len(CENTERS)-1);pg=[]
    for eta in (.25,.5,.75,1.):
        pp=pooled(p,prior,eta);pg.append({'eta':eta,'CE':float(-np.log(pp[np.arange(len(vt)),targetbin].clip(1e-300)).mean())})
    ck['eta']=min(pg,key=lambda r:(r['CE'],abs(r['eta']-1)))['eta'];pp=pooled(p,prior,ck['eta'])
    torch.save(ck,out/'validation_selected_model.pt');np.savez_compressed(out/'validation_predictions.npz',p=pp,pH=p[:len(vm)],pL=p[len(vm):],outside_H=ood[:len(vm)],outside_L=ood[len(vm):],group=vg,truth=vt)
    dev.csv_write(out/'history.csv',pd.DataFrame(history));dev.csv_write(out/'temperature_grid.csv',pd.DataFrame(grid));dev.csv_write(out/'pooling_grid.csv',pd.DataFrame(pg))
    report={'deployment':dep,'seed':seed,'epoch':ck['epoch'],'temperature':ck['temperature'],'eta':ck['eta'],
        'network_logMc_MAE':float(abs(pp@CENTERS-vt).mean()),'seconds':time.perf_counter()-start,'checkpoint_sha256':dev.sha(out/'validation_selected_model.pt')}
    dev.json_write(out/'COMPLETE.json',report);print(json.dumps({'detector_mass_done':report}),flush=True)
...
def prediction(root,dep,ms,es,split):
    ...
    path=root/f'detector_mass_density/predictions/{dep}/model_{ms}_eval_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
    ...
    x=matrix(coarse,extra);model=Predictor().cuda().eval();model.load_state_dict(ck['model']);p,ood=probability(infer(model,(x-ck['mu'])/ck['sd']),ck,ck['temperature']);n=len(coarse)
    a={'p':pooled(p,ck['prior'],ck['eta']),'pH':p[:n],'pL':p[n:],'outside_H':ood[:n],'outside_L':ood[n:]}
    if split=='real':
