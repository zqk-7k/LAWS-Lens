...
DEFAULT_PACKAGE = REPO / "packages" / "real_noise_injection_v3_physical_20260721_deliverables.tar.gz"
...
def make_candidate_figure(root: Path, figure_dir: Path) -> None:
    ...
    for row, (deployment, label) in enumerate(DEPLOYMENTS.items()):
        path = root / deployment / f"seed_{SEEDS[0]}" / "results" / "real_pair_scores_strict_h1l1_bbh_three_channel_strictly_positive.parquet"
        ...
        ax.set_title(f"{label}: strict H1L1 top 10")
        ax.axvline(0, color="black", lw=0.6)
        if row == 0:
            ax.legend(ncol=3, fontsize=8)
        ax = axes[row, 1]
        stability = pd.read_csv(root / deployment / "real_candidate_rank_stability.csv")
        subset = stability[(stability.scope == "strict_h1l1_bbh") & (stability.method == "three_channel_strictly_positive")].head(20)
...
def deployment_result(root: Path, deployment: str) -> dict[str, Any]:
    ...
    weights = pd.read_csv(root / deployment / "fusion_weights_per_seed.csv")
    stability = pd.read_csv(root / deployment / "real_candidate_rank_stability.csv")
    strict = stability[(stability.scope == "strict_h1l1_bbh") & (stability.method == "three_channel_strictly_positive")].head(10)
    metrics = {}
    ...
    return {
        ...
        "top10_consensus_strict_positive": strict.to_dict(orient="records"),
...
def build_report(summary: dict[str, Any], chinese: bool) -> str:
    ...
    for key in DEPLOYMENTS:
        ...
        top_rows = [[int(row["median_rank"]), row["event_i"], row["event_j"], f"{row['q25_rank']:.1f}-{row['q75_rank']:.1f}", f"{row['top10_frequency']:.2f}"] for row in item["top10_consensus_strict_positive"]]
    ...
    if chinese:
        lines += [
            ...
            "",
            "PE 内禀参数一致性是冻结排名后的独立筛查。若榜首不通过 PE，结论是当前检索没有给出可信 follow-up candidate，而不是在真实榜首上继续调权。",
            "",
        ]
    else:
        lines += [
            ...
            "",
            "PE intrinsic-parameter consistency is an independent screen after ranking is frozen. A failed PE screen means that the current retrieval produced no credible follow-up candidate; it is not a license to retune on the real catalog.",
            "",
        ]
    lines += [
        "## References",
...
/root/miniconda3/bin/python scripts/experiments/20_real_noise_injection_v3_physical.py \\
  --out-root results/real_noise_injection_v3_physical_20260721 \\
  --deployments GWTC3 GWTC4 \\
  --seeds 202607211 202607212 202607213 \\
  --samples-per-family 600 --epochs 20 --batch-size 8 --bootstrap-draws 10000
/root/miniconda3/bin/python scripts/experiments/19_validate_physical_scoring_invariants.py
/root/miniconda3/bin/python scripts/experiments/23_waveform_domain_shift_audit.py
/root/miniconda3/bin/python scripts/experiments/21_physical_deployment_pe_audit.py
/root/miniconda3/bin/python scripts/experiments/22_make_physical_deployment_report.py
"""
...
def package(root: Path, package_path: Path) -> None:
    ...
    explicit = [
        ...
        REPO / "scripts" / "experiments" / "21_physical_deployment_pe_audit.py",
        REPO / "scripts" / "experiments" / "22_make_physical_deployment_report.py",
        REPO / "scripts" / "experiments" / "23_waveform_domain_shift_audit.py",
