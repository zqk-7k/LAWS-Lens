...
def main():
    ...
    receipt = root/'contracts/REGISTERED_PREFETCH_RUN_ENDPOINT_TRANSPORT.json'
    if not receipt.exists():
        b.n.write_json(receipt, {'UTC': b.n.utc(), 'transport_only': True,
            ...
            'metadata': 'gwosc.locate.get_run_urls for the explicitly frozen run, avoiding an unnecessary slow all-event search; returned run/clock/sample-rate checked by unchanged generator.',
        ...
        shutil.copy2(__file__, root/'scripts/bulk_noise_prefetch_ranges_v2.py')
    urls = []
    for row in plan.itertuples():
        for det in ('H1', 'L1'):
            values = get_run_urls(row.run, det, row.query_gps, row.query_gps+1, sample_rate=4096)
            if len(values) == 1:
                urls.append(values[0])
            print('REGISTERED_PREFETCH_URL', int(row.proposal), det, len(values), flush=True)
