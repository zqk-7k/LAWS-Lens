...
import mcwf_mixture_density_20260907 as previous

dev, body = e.dev, e.body
torch.set_num_threads(2)
SEEDS = (202609231, 202609232, 202609233)
K, D, EPOCHS = 4, 3, 60


class Density(nn.Module):
    def __init__(self, dim=15855):
        super().__init__()
...
def initialize(root):
    ...
    dev.json_write(path, {'created_utc': datetime.now(timezone.utc).isoformat(), 'same_O3_O4a': True,
        ...
        'input': 'H1L1 peak2s4096points physical40-580Hz; event-PSD phase-quadrature log1p power from3584existingDplus1701intermediate-McDtemplates;253Mcgridcentersatoriginalq/spingrid; no metadata as network inputs',
        'network': '15855->256 LayerNorm SiLU dropout0.1->128 SiLU->4 full-covariance3D Gaussian components',
...
def inputs(root, dep, split):
    old=previous.inputs(root,dep,split)
    extra=fine.development(root,dep,split)
    return np.concatenate([old,extra.reshape(len(extra),-1)],1)
...
def train(root, dep, ms, seed):
    initialize(root)
    out = root / f'fine_mass_density/models/{dep}/seed_{ms}'
    ...
    for epoch in range(first, EPOCHS+1):
        ...
        if epoch % 10 == 0:
            print(json.dumps({'fine_mass_training': dep, 'seed': seed, **history[-1]}), flush=True)
    ...
    print(json.dumps({'fine_mass_training_done':report}),flush=True)


def prediction(root,dep,ms,es,split):
    cp=root/f'fine_mass_density/models/{dep}/seed_{ms}/validation_selected_model.pt'
    if not (cp.parent/'COMPLETE.json').exists():
        raise RuntimeError('Density training incomplete')
    path=root/f'fine_mass_density/predictions/{dep}/model_{ms}_eval_{es}/{split}.npz'
    if path.exists():
        a=np.load(path)
    ...
    x=np.concatenate([x,refined.reshape(len(refined),-1)],1)
...
if __name__=='__main__':
    ...
    initialize(a.root);dev.json_write(a.root/'fine_mass_density/contracts/UNIT_TESTS.json',tests())
    for dep in e.DEPS:
