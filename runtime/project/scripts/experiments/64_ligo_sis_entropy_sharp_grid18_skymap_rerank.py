...
OUT_ROOT = Path('runs/ligo_sis_entropy_sharp_grid18_skymap_rerank_20260605')
...
def train_model(job, train_ds, train_raw, val_ds, val_raw, out_dir):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x, y, unit in train_loader:
            ...
            entropy = -torch.mean(torch.sum(prob * torch.log(torch.clamp(prob, min=EPS)), dim=1))
            loss = loss_kl + ANGULAR_LAMBDA * loss_ang + ENTROPY_LAMBDA * entropy
...
def run_job(job):
    ...
    row = {
        ...
        'method': 'ligo_sis_entropy_sharp_grid18_skymap_rerank',
        'features': 'log1p_delta_time_obs,entropy_sharp_grid18_skymap_overlap,waveform_score,waveform_reciprocal_rank',
        'sky_model': f'SkyMapCNN_entropy_sharp_grid_{DEC_BINS}x{RA_BINS}',
