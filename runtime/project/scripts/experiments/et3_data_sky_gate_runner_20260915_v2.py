...
def parallel_maps(base, root, q, workers):
    import pandas as pd
    config = json.loads((root/'contracts/ANALYSIS_CONTRACT.json').read_text())
    def run(index):
        base.guard(root)
        case = {'idx': index, 'q': q, 'id': f'event{index:02d}_q{q}'}
        folder = root/'maps'/case['id']
        folder.mkdir(exist_ok=False)
        base.write(folder/'CASE.json', case)
        started = time.perf_counter()
        command = [sys.executable, '-B', '-u', str(root/'scripts/et3_data_sky_pilot_20260915.py'),
                   '--root', str(root), '--case', str(folder/'CASE.json')]
        with (folder/'stdout.log').open('wb') as stdout, (folder/'stderr.log').open('wb') as stderr:
            try:
                process = subprocess.run(command, stdout=stdout, stderr=stderr,
                                         timeout=config['timeout_seconds_by_q'][str(q)], check=False)
                result = {**case, 'state': 'COMPLETE' if process.returncode == 0 else 'FAILED',
                          'exit_code': process.returncode}
            except subprocess.TimeoutExpired:
                result = {**case, 'state': 'TIMEOUT'}
        result.update({'wall_with_startup_seconds': time.perf_counter()-started,
                       'workers': workers, 'threads_per_map': 4})
        base.write(folder/'STATUS.json', result)
        return result
    done = []
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(run, i) for i in range(config['development_event_count'])]
        for future in as_completed(futures):
            done.append(future.result())
            base.write(root/'STATUS.json', {'state': 'DATA_Q128_RUNNING', 'q': q, 'workers': workers,
                'finished': len(done), 'total': config['development_event_count'],
                'failed': sum(x['state'] != 'COMPLETE' for x in done), 'utc': base.utc()})
            print(json.dumps(done[-1]), flush=True)
    pd.DataFrame(done).to_csv(root/'tables'/f'JOBS_Q{q}.csv', index=False)
    base.write(root/'contracts'/f'DATA_SKY_Q{q}_RESULT.json', {
        'state': 'DEVELOPMENT_MAPS_ONLY_NO_PRODUCTION_ADOPTION', 'q': q,
        'maps_complete': sum(x['state'] == 'COMPLETE' for x in done), 'workers': workers,
        'oracle_template': False, 'full_BBH_PE': False, 'encoder_training_started': False,
        'bulk_started': False, 'heldout_test_opened': False})


def main(root, oracle, deliver):
    ...
    base.write(lock, {'utc': base.utc(), 'pid': os.getpid(), 'code_sha256': base.sha(__file__),
        ...
        'delivery_script_sha256': base.sha(project_scripts/'et3_repair_delivery_20260915.py') if deliver else None})
    shutil.copy2(__file__, root/'scripts'/Path(__file__).name)
    base.write(root/'contracts/RESOURCE_SCHEDULE_ADDENDUM.json', {
        'utc': base.utc(), 'q64_workers_unchanged': 2, 'q128_workers': 6,
        'threads_per_map_unchanged': 4, 'maximum_compute_threads': 24, 'CPU_quota': 25,
        'q128_waits_for_oracle_stage_completion': True,
        'reason': 'reuse the 16 CPU threads released by the oracle control; do not oversubscribe the 25-core quota',
        'scientific_inputs_priors_precision_quadrature_and_all_Gates_unchanged': True,
        'historical_contract_not_overwritten': True})
    ...
    if not g64['pass']:
        start = time.monotonic()
        while not (oracle/'contracts/FINAL_RESULT.json').exists():
            if time.monotonic()-start > 10800:
                raise RuntimeError('Oracle stage did not release its resources within wait bound')
            time.sleep(30)
        ...
        base.write(root/'contracts/Q128_LAUNCH.json', {'utc': base.utc(),
            'case_worker': str(root/'scripts/et3_data_sky_pilot_20260915.py'),
            'threads_per_map': 4, 'workers': 6, 'frozen_gate': g64,
            'scientific_configuration_unchanged': True})
        parallel_maps(base, root, 128, 6)
    ...
    if deliver:
        expected = json.loads(lock.read_text())['delivery_script_sha256']
        if base.sha(project_scripts/'et3_repair_delivery_20260915.py') != expected:
            raise RuntimeError('Delivery script changed after launch')
        while not (oracle/'contracts/FINAL_RESULT.json').exists():
            time.sleep(30)
        oracle_q = 128 if len(list((oracle/'maps').glob('*q128/RESULT.json'))) == 24 else 64
        oracle_commands = [['et3_pixel_coverage_audit_20260915.py', '--q', str(oracle_q)]]
        if oracle_q == 128:
            oracle_commands.append(['et3_rank_stability_20260915.py', '--qa', '64', '--qb', '128'])
        for name, *extra in oracle_commands:
            target = oracle/'scripts'/name
            if not target.exists():
                shutil.copy2(project_scripts/name, target)
            subprocess.run([sys.executable, '-B', str(target), '--root', str(oracle), *extra], check=True)
        contract = json.loads((root/'contracts/ANALYSIS_CONTRACT.json').read_text())
        command = [sys.executable, '-B', str(project_scripts/'et3_repair_delivery_20260915.py'),
            '--root', str(oracle), '--reference', contract['reference'], '--bank', contract['bank_source'],
            '--data-sky', str(root)]
        subprocess.run(command, check=True)
        base.write(root/'contracts/DELIVERY_COMPLETE.json', {
            'utc': base.utc(), 'state': base.FINAL,
            'audit': str(oracle/'manifest/FINAL_PACKAGE_AUDIT.json')})
...
if __name__ == '__main__':
    ...
    try:
        main(args.root, args.oracle, args.deliver)
    except Exception:
