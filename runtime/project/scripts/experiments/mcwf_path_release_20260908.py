...
def export(root):
    ...
    shutil.copy2(__file__, root/'scripts/path_release.py')
    dev.json_write(root/'contracts/EXPORT_IMPLEMENTATION_FREEZE.json', {
        ...
        for seed in t.SEEDS:
            for split in ('validation', 'test', 'real'):
                ...
                for name, f, ww in (('OMC', b, w0), (CODE, c, w)):
                    ...
                    f['waveform_contribution'] = f.waveform_score*ww[0]
                    f['time_contribution'] = f.time_score*ww[1]
                    f['sky_contribution'] = f.sky_raw_log_bf*ww[2]
                    ...
                    for channel in ('waveform_score', 'time_score', 'sky_raw_log_bf'):
                        groups = [('all_pairs', f)] if split == 'real' else [('true', f[f.is_true_pair]), ('null', f[~f.is_true_pair])]
