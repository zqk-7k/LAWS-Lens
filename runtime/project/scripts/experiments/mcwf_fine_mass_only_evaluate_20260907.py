...
import mcwf_fine_mass_only_20260907 as mass_model
...
def calibrate(root, dep, ms):
    path = root / f'fine_mass_only/calibration/{dep}/seed_{ms}/CALIBRATION.json'
    if path.exists():
...
def run(root, arm):
    trial = root / f'trials/FINE-MC-ONLY-{arm}'
    if trial.exists():
    ...
    dev.json_write(trial / 'contracts/RECIPE.json', {
        ...
        'arm': arm, 'model': 'joint H1L1 fine-Mc 15855 physical features, mass-only 4-Gaussian predictive density',
