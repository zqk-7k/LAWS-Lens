...
DEFAULT_INPUT = REPO / "results" / "real_noise_injection_v3_physical_20260721"
DEFAULT_OUT = REPO / "results" / "real_noise_injection_v3_physical_20260721" / "pe_followup"
...
def candidate_union(input_root: Path, deployment: str, method: str, top_n: int = 50) -> pd.DataFrame:
    rows = []
    for seed in SEEDS:
        path = input_root / deployment / f"seed_{seed}" / "results" / f"real_pair_scores_strict_h1l1_bbh_{method}.parquet"
...
def plot_top5_posteriors(deployment: Deployment, top: pd.DataFrame, cache: dict[str, dict[str, Any]], figure_dir: Path) -> None:
    ...
    for row_index, pair in enumerate(top.head(5).itertuples(index=False)):
        for col_index, parameter in enumerate(PARAMETERS):
            ...
            if col_index == 0:
                ax.set_ylabel(f"Consensus rank {int(pair.median_rank)}\nDensity")
...
def plot_waveform_inputs(input_root: Path, deployment: Deployment, top: pd.DataFrame, figure_dir: Path) -> None:
    ...
    for row_index, pair in enumerate(top.head(5).itertuples(index=False)):
        for detector, ax in enumerate(axes[row_index]):
            ...
            ax.set_title(f"Consensus rank {int(pair.median_rank)}, {'H1' if detector == 0 else 'L1'}")
            if detector == 0:
...
def main() -> None:
    ...
    parser.add_argument("--method", default="three_channel_strictly_positive")
    ...
    for key, deployment in DEPLOYMENTS.items():
        ...
        reference_path = args.input_root / key / f"seed_{SEEDS[0]}" / "results" / f"real_pair_scores_strict_h1l1_bbh_{args.method}.parquet"
        ...
        consensus = candidate_union(args.input_root, key, args.method, top_n=50)
        ...
        followup["pe_followup_screen_pass"] = followup["intrinsic_3sigma_consistent"].fillna(False) & followup["available"].fillna(False) & (followup["symmetric_log_evidence"].fillna(-np.inf) > 0)
    ...
    overall["methodology"] = {
        ...
        "screen_definition": "D <= 3 for Mc_det, q and chi_eff, plus approximate symmetric 3D posterior-overlap log evidence > 0",
