...
def build(root):
    target=root/'contracts/DATA_SPLIT_CONTRACT_v2.json'
    if target.exists():
    ...
    source_path=root/'tables/SOURCE_SPLIT_v2.parquet';m.to_parquet(source_path,index=False)
    source_csv=root/'tables/SOURCE_SPLIT_v2.csv';m.to_csv(source_csv,index=False,encoding='utf-8-sig')
    ...
    allowed={Path(x['path']).name for x in json.loads((root/'manifests/DOWNLOAD_JOBS.json').read_text()) if x['kind']=='strain'}
    by_parent={}
    for item in entries:
        ...
        if not all(Path(d['hdf5_url']).name in allowed for d in (h,l)):continue
    ...
    noise_path=root/'tables/NOISE_BLOCK_CANDIDATES_v2.parquet';noise.to_parquet(noise_path,index=False)
    bank_path=root/'tables/NOISE_BANK_SELECTION_v2.parquet';bank.to_parquet(bank_path,index=False)
    cfg={'code':'O4B-HL-NSO-01','created_utc':datetime.now(timezone.utc).isoformat(),
         ...
         'preflight_revision':'v1 included four noise parent windows outside the frozen download scope; v2 filters to contracted input files before hash selection. No data evaluation or model training occurred between revisions. v1 retained.',
    ...
    write(target,cfg)
    write(root/'contracts/DATA_SPLIT_FREEZE_v2.json',{'path':str(target),'sha256':sha(target)})
    print(json.dumps({k:v for k,v in cfg.items() if k!='index_split'},indent=2))
