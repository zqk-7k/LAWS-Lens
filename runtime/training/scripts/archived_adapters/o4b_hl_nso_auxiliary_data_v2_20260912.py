...
def modules(root):
    ...
    training=support.load(helpers.with_name('waveform_multiscale_train.py'),'o4b_aux_view')
    data.peak2s=training.make_window_view
    from scripts.real_search import physical_common as phys
    return src,data,phys


def initialize(root):
    p=root/'contracts/AUXILIARY_POPULATION_CONTRACT_v2.json'
    if p.exists():return
    ...
    support.write(p,{'UTC':support.now(),'purpose':'waveform-only coverage-balanced development for ordered Mc and multiscale conditional models',
        ...
        'environment_partition':'v2: filter eligible two-image environments first, then source-ID hash partition70/30 separately within smooth/subhalo; old v1 train plan and failed validation plan retained, no waveform generation or model fitting occurred under v1',
    ...
    support.write(root/'contracts/AUXILIARY_POPULATION_FREEZE_v2.json',{'sha256':support.sha(p)})


def plan(root,split):
    initialize(root);out=root/f'auxiliary_data_v2/{split}';out.mkdir(parents=True,exist_ok=True)
    ...
    for _,row in valid.iterrows():
        ...
        if images is not None and images['proposal_snr_ratio']<=4:eligible[bool(row.lens_is_subhalo)].append((row,images))
    environments={False:[],True:[]}
    for family,rows in eligible.items():
        ids=sorted({int(row.event_id) for row,_ in rows},key=lambda i:hashlib.sha256(f'O4bAuxEnvV2:{i}'.encode()).hexdigest())
        cut=int(.7*len(ids));selected=set(ids[:cut] if split=='train' else ids[cut:])
        environments[family]=[(row,images) for row,images in rows if int(row.event_id) in selected]
...
def system(row):
    ...
    for image,number in [('a',1),('b',2)]:
        ...
        for variant in range(views):
            ...
            short=data.peak2s(full[None],2)[0]
...
def generate(root,split,workers):
    ...
    out=root/f'auxiliary_data_v2/{split}';marker=out/'COMPLETE.json'
