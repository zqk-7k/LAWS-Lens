...
if __name__=='__main__':
    ...
    status=a.root/'RNC_RUN_STATUS.json';script='o4b_hl_nso_rankncontrast_20260912.py'
    try:
        c.s.write(status,{'state':'WAIT_GPU_FOR_RNC_FEATURES','utc':c.s.now(),'locked_test_opened':False})
        c.call(a.root,'rnc_features',script,['--stage','prepare'])
        for seed in c.SEEDS:
            c.s.write(status,{'state':'O4B_RNC_TRAINING','seed':seed,'utc':c.s.now(),'locked_test_opened':False})
            c.call(a.root,f'rnc_train_{seed}',script,['--stage','train','--seed',str(seed)])
